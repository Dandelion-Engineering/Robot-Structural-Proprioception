...
Many model-based robot controllers begin from a calibrated description of the body --- link lengths, masses, inertias, stiffness assumptions, and a model of how each motor responds to a command. Robust and adaptive controllers can tolerate or learn some mismatch, but the nominal model still matters for accurate prediction and control. The trouble is that real bodies do not hold still. Parts wear and loosen; joints heat up and develop friction or backlash; a link takes a knock and goes slightly soft; a motor gradually weakens. And the sensors that are supposed to be watching all of this have their own drift and noise. A robot that keeps relying on a stale model can become quietly inaccurate --- or unsafe --- while remaining perfectly capable, mechanically, of the task in front of it.
...
\subsection{Why it is hard, part 1: an observation map can have blind spots}

The first difficulty is the deepest, because sometimes it is not a matter of building a better estimator --- the measurements themselves do not contain a unique answer. For a rigid articulated robot, link mass, center of mass, and inertia map into the joint-space behavior observed through torques and joint motion. A 2024 result proves that this particular inertial-parameter map has a \emph{nullspace}: some redistributions of inertia --- the authors call them ``inertial transfers'' --- produce \emph{no change at all} in the standard joint-space dynamics (\href{https://doi.org/10.1177/02783649241258215}{Wensing, Niemeyer \& Slotine, 2024} --- gives the exact, provable characterization of which \emph{rigid-body inertial} parameter combinations are identifiable and which are not). No larger model can recover a parameter combination that never changes the data it receives.

The scope boundary matters. Wensing and colleagues prove this result for \emph{rigid-body inertial parameters}; they do not prove that this project's link-stiffness loss, actuator-gain loss, or encoder corruption is invisible to C1. We use their result for the narrower lesson that estimator capacity cannot repair an uninformative observation map. Distributed strain and curvature are a bet on changing that map by looking at the body from a physically different direction. Whether they actually separate \emph{our} three change types is not guaranteed by the theorem --- it is exactly what the feasibility spike and matched C1-versus-S experiment must establish.

\subsection{Why it is hard, part 2: a disagreement doesn't name its own cause}

The second difficulty is subtler and is the real technical heart of the project. Suppose a robot compares what it expected to sense with what it actually sensed, and the two disagree. That disagreement --- a ``residual,'' in the vocabulary of fault diagnosis --- tells the robot that \emph{something} is wrong, but not \emph{what}. A weakened motor, a softened link, and a lying joint-sensor can all produce overlapping disagreement patterns. Classical fault-diagnosis theory shows that cleanly separating these causes generally requires either special structural conditions or \emph{added redundancy} --- an extra, independent measurement that the ambiguous ones can be checked against (\href{https://doi.org/10.3390/s23146633}{Sravani \& Venkata, 2023} --- a process-control example, not a robotics proof, that shows how analytical redundancy distinguishes sensor faults from real disturbances and how difficult gradual drift remains).
...
The trap the spike guards against is \emph{circularity}. If the ``strain'' we feed the estimator is secretly just an algebraic re-encoding of the joint angles the conventional suite already has, then S has no independent information and any apparent win is an artifact. The defense is to derive the strain from \emph{independent deformation coordinates} --- degrees of freedom the simulator integrates from applied forces, physically distinct from the joint coordinates --- and then cross-check them against a completely independent physics calculation. Our primary engine candidate is MuJoCo, a fast, free, widely-used physics engine. Its generic 1-D flex consists of line elements and does not automatically provide beam-bending elasticity or a strain sensor; its first-party \href{https://mujoco.readthedocs.io/en/stable/programming/extension.html\#elasticity}{elasticity extension} separately provides a 1-D cable model with decoupled bending and twisting strain. The spike therefore tests that bending-capable cable model, and a slender 3-D solid only if needed, rather than assuming any object called a ``flex'' will do.

If the native mechanics cannot produce clean, independent, above-the-noise fault signatures at realistic (metal-like) stiffness, we fall back to a Cosserat-rod model, a well-established way to simulate the bending and twisting of slender rods (\href{https://github.com/GazzolaLab/PyElastica}{PyElastica} --- an open implementation that can cross-check a MuJoCo plant or become the fallback plant). A model cannot validate itself: if PyElastica becomes the plant, its gauge outputs still need a separate small-deflection beam calculation or offline finite-element check. Heavy finite-element simulation stays offline, used only to generate ground truth and validate the cheaper models, never inside the live control loop. This is the ``fidelity ladder'' in the Claim Sheet: native rod, then reduced-order Cosserat, then offline finite-element or analytical ground truth.
...
A fault call is only useful if the robot knows \emph{when to trust it}. Acting confidently on a wrong diagnosis --- reconfiguring the controller for a motor fault that is actually a sensor glitch --- can be worse than doing nothing. So attribution in this project is explicitly \emph{staged and calibrated}: detect that something changed; classify it as structure, actuator, or sensor --- \emph{or abstain} and say ``I'm not sure''; localize and estimate severity where possible; and only then use the estimate for recovery. The ability to abstain is treated as first-class behavior because an honest ``unknown'' is often the safe action. But on a confirmatory run whose cause is one of the four known classes, abstention still counts as an error in the headline score; safety is not allowed to become a scoring loophole.
...
The headline diagnosis number is \emph{four-way macro-F1} across the classes \{healthy, structure, actuator, sensor\}, with balanced accuracy as a companion check. F1 is a standard accuracy score that combines two kinds of mistake --- false alarms and misses --- into a single value between 0 (worst) and 1 (best); ``macro'' means we average it evenly across the four classes so that a rare cause counts as much as a common one, and the robot cannot look good just by nailing the easy majority class (\href{https://scikit-learn.org/stable/modules/generated/sklearn.metrics.f1_score.html}{scikit-learn's f1\_score documentation} --- a precise, runnable definition of F1 and macro-averaging). Around that headline sit per-class precision, recall, and confusion tables; detection delay; localization error; and --- because a fault call needs honest confidence --- calibration, measured with standard scores and reliability diagrams (\href{https://scikit-learn.org/stable/modules/calibration.html}{scikit-learn's probability-calibration guide} --- explains reliability diagrams and what ``well-calibrated'' means operationally). Selective risk-versus-coverage and false abstention are measured separately, and the never-before-seen compound fault tests whether the system can flag a genuinely unfamiliar situation as unknown rather than forcing it into one of the four boxes.

\subsection{The control layer}

The headline control number is the \emph{five-second post-change integral of absolute tracking error}. In words: over the five seconds right after a change is applied, add up how far the arm is from where it should be at each instant. The generic mathematical shape makes the time accumulation explicit:
\[
J_{5\text{s}} \;=\; \int_{t_c}^{\,t_c + 5} \bigl\lVert e(t) \bigr\rVert \, dt,
\]
where $t_c$ is the instant the change happens, $e(t)$ is the tracking-error vector at time $t$, $\lVert \cdot \rVert$ turns that vector into a single non-negative size, and the integral accumulates that size over the five-second recovery window. Lower is better --- it means the arm spent those five seconds closer to correct. Before confirmatory generation, the shared schema and frozen configuration must name the tracking space, units, norm, sampling rule, and numerical integration rule; the equation must not silently choose those implementation details after results exist. The success bar asks S to cut the frozen quantity by at least 10\% relative to C1. Alongside it we report tracking error before, during, and after adaptation, recovery time, how much of healthy performance is regained, and any unsafe or constraint-violating excursions, because a ``recovery'' bought by dangerous motion is not a recovery.

\subsection{Leakage, frozen choices, and paired uncertainty}
...
\begin{itemize}
  ...
  \item \textbf{The bar was set before the result.} A movable bar can always be cleared. \emph{Guarded by} pre-specification, a versioned configuration freeze, and the pilot-sizes-not-chooses rule.
\end{itemize}
