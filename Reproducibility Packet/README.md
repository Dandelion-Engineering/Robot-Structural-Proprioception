...
## Step 17 — Reproduce the class-probability channel screen

Runs after Steps 14 and 15, whose committed rows it reads. Step 16 closes the severity route at the `0.50` boundary and names the remaining one explicitly: the recovery controller weights its correction by the estimated class probability, and every arm in every prior screen pins that probability at one. This screen measures that channel, at the condition Step 14 actually **selected** — `actuator_gain_remaining_0p25` — rather than at Step 16's boundary.

Two structural facts make the measurement exact rather than sampled. First, the compensation is `min(1 / max(severity, minimum_gain_remaining), maximum_gain_compensation)`, so at the recorded cap **every severity estimate at or below `0.50` yields the identical capped compensation** — the severity channel is structurally flat here, and the true `0.25` sits about 25x its recorded error scale inside that flat region. Second, the probability channel is closed at *both* ends by recorded controller constants: the confidence gate at `source_probability_threshold` below, the cap above. The reachable commanded multiplier is therefore exactly `[1.50, 2.00]`, and the sweep covers the whole of it. This is a **reachable-set** span, not a chosen-range envelope, and the distinction is stated in the artifact rather than assumed.
...
Reported quantities are in the contract's own units — `100 x (J_C1 - J_S) / J_C1` — not in differences of no-action reductions, which are smaller by `1 / (1 - r_low / 100)` and would understate the quantity the bar is written in. A sub-threshold probe that keeps the actuator as the unique argmax verifies that the gate withholds the action entirely, so the gate discontinuity is measured rather than inferred. CRN reuse against Step 14's committed rows, the one-decision lifecycle, withheld/acting behavior, A1 safety, saturation, and the commanded-versus-applied multiplier identity are all fail-loud gates.
...
The recorded outcome **closes the class-probability channel** on the actuator class at the selected condition. The widest paired difference two gate-clearing suites can produce through it is **5.07 percentage points** (mean 5.02), against a 10-point bar — and that is across the channel's entire reachable set, so no probability read-out, calibrated or not, reaches the bar this way without also crossing the confidence gate. The gate crossing itself is reported separately at **10.82 points**, because one suite withholding the action while the other acts at full confidence is an authorization difference, not a probability-precision one; both suites call this class correctly with one-hot recorded probabilities, so neither is currently in play.

Two further recorded results. The action's benefit is real and steeply cap-limited: mean no-action deficit **23.16%**, analytic exact-restoration ceiling **18.81%**, realized at the cap **10.82%** — **57.5% of the ceiling**, in the same direction on every seed. That is a far larger shortfall than Step 16's 93.2% at the `0.50` boundary, and it is attributable to cap saturation rather than to pre-decision error alone; `maximum_gain_compensation` is therefore the binding limit on recoverable tracking at the condition the action screen will run on. And the gate receives a **bias-inclusive RMS** severity error scale rather than a residual standard deviation, because a standard deviation discards the bias and would pass a systematically wrong but tightly clustered read-out; the conservative suite value clears the gate by 25x, so no arm is sensitive to the choice.

This is development-sized evidence on four assessment seeds, one bounded condition, one fault location and setting, held out over sensor noise only, at an unfrozen config. It does not close the actuator class — action-versus-no-action benefit, healthy false authorization, cap and floor sensitivity, and the source-specific margin belong to the action screen — and it does not extend to a different cap, since both the flat severity region and the reachable multiplier set are functions of `maximum_gain_compensation`.
