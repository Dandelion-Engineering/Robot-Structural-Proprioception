...
The recorded outcome is that **severity-estimation quality does not open a control path on the actuator class, for two independent reasons.** First, at the recorded compensation cap of 2.0 the multiplier's flat region is every estimate at or below 0.5, and no severity on the grid is both severity-sensitive and above the bar: 0.85 and 0.70 remaining gain are severity-sensitive but their exact-restoration ceilings are 2.62% and 5.91%, while 0.50, 0.25, and 0.10 clear the bar but sit in the flat region. The smallest cap admitting any reachable severity is 3.0, and it admits only 0.50; `minimum_gain_remaining = 0.25` bounds the sensitive interval from below, so the cap and the floor are a joint constraint rather than a single tunable. Second, both suites already estimate remaining gain almost exactly — held-out mean absolute error **0.0060 for C1 and 0.0080 for S** — so the 32 additional gauge feature columns cost a little accuracy rather than adding any. This is expected mechanically: C0 already carries commanded actuation, the fault acts downstream of it, and commanded torque with the resulting encoder motion brackets the remaining gain directly. Pushed back through the real multiplier, the two suites command **identically on all 12 capped-region arms** at the recorded cap, both matching an oracle exactly; they differ on 15 of 28 arms overall, but by a mean absolute multiplier difference of 0.0096 and only on settings whose ceiling is below the bar. At caps of 4 and above the flat boundary coincides with the 0.25 floor and real estimates straddle it, so 4 of 8 capped-region arms differ there — a boundary artifact of reciprocal cap/floor values, not an information advantage. On the healthy arms S reproduces the no-action command on 75% of arms against C1's 25%, which is a false-authorization difference rather than a control-bar one. This is development-sized evidence about a linear read-out with `p` pinned at 1 for both suites; it is not validation-sized, not frozen-config, not evidence about the structural or sensor classes, and it does not measure the class-probability channel, which remains a separate untested route to a paired difference.

## Step 16 — Run the plant-interface and sensor-model tests
...
## Step 17 — Generate a real privileged trace from the selected MuJoCo plant
...
Use `--scenario structure --fault-severity 0.50` or `--scenario actuator --fault-severity 0.70` for a physical-fault development trace. Sensor faults are rejected here and must be injected only in Step 18.

## Step 18 — Apply the sensor-realism + fault-injection model
...
## Step 19 — Generate the optional analytic plant fixture
