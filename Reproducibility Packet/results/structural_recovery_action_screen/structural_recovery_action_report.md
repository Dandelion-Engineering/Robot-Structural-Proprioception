...
- **The structural fault's own tracking deficit bounds any structural recovery.** Its no-action `J_5s` sits +0.0494% from the healthy no-action arm's — only 0.18x the widest within-source seed spread, so it is not resolved above seed noise. A source-specific action can at best recover that gap, which is 203x smaller than the 10% gate it would have to clear. On this bounded condition the binding constraint on the control layer is the task/fault severity, not the action family and not the nominal controller's tuning.
- **The source-specificity margin is smaller than the spread of its own inputs.** It is the difference of two unpaired 4-seed means: -0.263 percentage points, against per-seed reduction spreads of 1.002 pp (structure) and 1.298 pp (healthy). No uncertainty is computed for it, so its sign is not established by this design and the block should not be read off it.
- **Most of the benefit comes from the joint carrying no fault** — this is the robust evidence for the block. At the identical 2.00x multiplier, `localized_2p00` applied only at the joint the diagnosis localizes recovers 6.16% against `global_2p00`'s 20.37%, so 70% of the improvement is produced at the unfaulted joint. That contrast is a within-role comparison at ~14 pp effect size and is not noise-limited.
- **The safety readouts did not discriminate between candidates.** No arm in the family raised an A1 flag or saturated, but the limits sit far from the operating point, so those gates carried no information here — and the selected action is not free of cost: mean peak contact force rises 2.084 -> 3.054 N (42% -> 61% of the 5.0 N limit) and mean peak structural |gauge| rises 37.9 -> 56.9 µε on the link the diagnosis says has lost stiffness. A threshold-crossing count cannot score that trade in either direction.
