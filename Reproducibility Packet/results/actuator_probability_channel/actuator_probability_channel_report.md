...
Development-sized screen. Measures the last channel through which two suites that both call the actuator class correctly could still command different recovery actions, at the condition the deficit screen selected.
...
- 36 arms; swept probabilities [0.6, 0.7, 0.8, 0.9] plus the gate endpoint, the certain endpoint, and a sub-threshold probe
- Config hash `dev-actuator-probability-channel-screen` (not frozen)

## Part 0 — why probability is the only live channel here
...
- The gate receives a **bias-inclusive RMS**, not a residual standard deviation. A standard deviation discards the bias, so a systematically wrong but tightly clustered read-out would pass a gate it should fail. The conservative suite value (0.010183) is used at every acting arm, and it clears the 0.25 gate by 25x, so no arm is sensitive to the choice.

## Part 1 — the reachable probability set

Both ends of this interval are recorded controller constants, not chosen grid points. The confidence gate closes it below at p = 0.5; the compensation cap closes it above. The reachable commanded multiplier is therefore exactly **[1.50, 2.00]**.
...
- **Across the entire reachable set p in [0.50, 1.00] the reduction moves by 4.7097 percentage points**, against a 10% bar — below it. The slope at the certain end is +8.1684 points per unit of probability.
...
The table below is in **the contract's own units** — `100 x (J_C1 - J_S) / J_C1`, the conventional suite in the denominator — not in differences of no-action reductions. Those are not the same number: a difference of two reductions is smaller by `1 / (1 - r_low / 100)`, so quoting the reduction span against the bar would understate the quantity the bar is written in. Here the reduction span is 4.7097 pp while the paired quantity it corresponds to is 5.0699 pp.
...
**These must not be collapsed into one number.** The graded span is what a *probability-precision* difference between two suites can buy. The gate-entry jump is what a difference in whether the suite authorizes the action at all can buy, and that is a class-call-scale quantity, already screened elsewhere: both suites call this class correctly with one-hot recorded probabilities, so neither the recorded difference nor the gate crossing is currently in play.
...
- **It closes the class-probability channel on the actuator class at this condition.** The widest paired difference two gate-clearing suites can produce through this channel is 5.0699 pp against a 10 pp bar, and that is over the channel's *entire reachable set*, closed at both ends by the controller's own recorded constants rather than by a chosen grid. No probability read-out — linear, learned, or calibrated — can reach the bar through this channel here without also crossing the confidence gate.
- **It does not license collapsing the gate into the channel.** A suite difference large enough to put one suite below the confidence threshold and the other above it is a class-call/authorization difference, and it is measured separately above.
