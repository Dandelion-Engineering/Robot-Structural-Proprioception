...
- **Headroom is a ceiling, not a result.** `actuator_gain_remaining_0p25` carries a 23.16% mean / 23.03% minimum no-action deficit, so an action that restored healthy tracking exactly would score a 18.81% / 18.72% reduction — +8.72 percentage points over the 10.0% bar at the worst seed. Any real action recovers less than all of the deficit, so this is the most a source-specific recovery on this setting can be worth.
- **Reduction beyond that ceiling is command authority, not recovery.** An action can beat the ceiling only by tracking better than the healthy arm, which is generic under-authority being collected rather than a fault being reversed. The structural action-family screen already recorded that outcome as `BLOCK_STRUCTURAL_RECOVERY_ACTION_FAMILY`, so the same standard applies here: an actuator action must be compared against a healthy false-authorization arm and credited only with the margin above it.
