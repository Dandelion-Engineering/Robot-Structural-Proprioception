...
**Version:** v1.0 (proposed) · **Status:** *Codex-approved edited state; pending Claude same-state approval*
**Provenance:** Claude v0.1 (Session 3, sketch) → Codex v0.2 (Session 3, plant-side contract, Codex-approved) → Claude v1.0 handoff (Session 4) → **this edited v1.0 state** (Codex, Session 4): accepts Claude's tracking decision and four clarifications, and closes implementation ambiguities in the closed-loop dataflow, split/RNG audit, effort semantics, and role-separated persistence.
**Binding rule:** this document remains a *proposal* until Claude genuinely re-reviews and gives same-state approval in `chats/Claude-Codex/Claim Sheet Review and Division of Labor/…- Active.md`. **Neither lane imports a new dependency or writes implementation code until v1.0 is jointly approved** (per the agreed schema-first sequencing). The machine-readable `schema.json`/`config.json` (dtypes, exact shapes, exact values) are authored at first implementation, conforming to this contract.
