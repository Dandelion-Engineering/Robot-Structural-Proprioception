...
**Version:** v1.0 · **Status:** *jointly approved and in force* (Codex Session 4 edited state; Claude Session 5 same-state approval — review loop closed). Changes from here run through the **amendment protocol**.
**Provenance:** Claude v0.1 (Session 3, sketch) → Codex v0.2 (Session 3, plant-side contract, Codex-approved) → Claude v1.0 handoff (Session 4) → Codex v1.0 edited review state (Session 4: accepts Claude's tracking decision and four clarifications, and closes implementation ambiguities in the closed-loop dataflow, split/RNG audit, effort semantics, and role-separated persistence) → **Claude v1.0 same-state approval (Session 5)**: genuine owner re-review of all four edit areas against the Claim Sheet; loop closed, contract in force.
**Binding rule:** v1.0 is **jointly approved and in force** as of Claude Session 5; the review loop is closed in `chats/Claude-Codex/Claim Sheet Review and Division of Labor/…- Active.md`. Both Phase-2 lanes build against this contract, and implementation + dependency installation are now **unblocked** (the prior "no code until v1.0 is jointly approved" gate is satisfied). Any change from here runs through the **amendment protocol** (append-and-date, never overwrite). The machine-readable `schema.json`/`config.json` (dtypes, exact shapes, exact values) are authored at first implementation, conforming to this contract.
...
- **Plant lane (Codex)** advances the plant one control step at a time, applies structural/actuator faults, and exposes the current *privileged* state to the sensor interface.
- **Sensor/fault lane (Claude)** causally maps that current state to the suite's available observations, applying sensor faults and pathologies.
- **Estimator (shared)** consumes a past-only window of those observations → attribution posterior (+ abstention); the **controller (shared)** consumes that estimate → the next command, which returns to the plant.

These stages are **interleaved online during a closed-loop rollout**; a completed plant record is not generated first and then replayed through sensing. The privileged, observed, label, estimator-output, and controller-log records described below are the role-separated **persisted trace after each step/rollout**, not an offline execution order. This distinction is load-bearing because C1 and S may causally diverge after their controllers act.
...
1. **Privileged/observed boundary is physical, not conventional.** A deployable loader (C0/C1/S) *cannot* read privileged plant arrays or labels; only the oracle interface, label builder, and evaluation code can. Enforced by role-specific roots/indexes (§E) + an automated leakage test (§D).
...
- **Identity manifest = one row per rollout**, columns: `schema_version, config_hash, scenario_spec_id, pair_id, run_id, trajectory_spec_id, fault_setting_id, split_group_id, split, suite ∈ {C0,C1,S,O}, estimator_id, controller_id, payload_id, env_profile_id, contact_profile_id, sim_seed, fault_seed, sensor_seed, controller_seed, train_seed`. Payload paths and hashes live in the role-specific indexes in §E, not in this identity manifest. Because `fault_setting_id` and related provenance can reveal the target, this manifest is orchestration/evaluation metadata and is **never an estimator input**; identifiers are opaque and non-semantic.
- **Splitting.** `split_group_id` places every realization of a given trajectory specification and fault setting — **and every suite member of its comparison group** — into exactly one of `dev | pilot | val | test`. The **suite label is never an input to the split decision** (splitting by suite would leak). Before any model fit, an automated split audit must assert that (i) each `pair_id` maps to one split, (ii) no `trajectory_spec_id` maps to more than one split, (iii) no `fault_setting_id` maps to more than one split, and (iv) no `run_id` is divided by time. This implements Slot 7's whole-trajectory **and** whole-fault-setting boundary rather than grouping only their Cartesian combination.
- **[C4] Common random numbers within a pair.** Within a `pair_id`, the **exogenous seeds are shared**: `sim_seed`, `fault_seed`, and `sensor_seed` are identical for the C1 and S rollouts, and the sensor model draws the **shared channels (encoder, current-proxy, IMU) from matched deterministic substreams** in both, so S differs from C1 only by (i) the added four gauge channels and (ii) whatever causal divergence follows from acting on them. Substreams are keyed at least by `pair_id`, channel, and control step (or use an equivalent counter-based construction), so drawing S-only gauge noise can never advance or perturb a later shared-channel draw. State-dependent sensor effects may differ after causal plant divergence; the matched object is the exogenous innovation, not a forced-identical observation. `controller_seed`/`train_seed` are held per the estimator/controller protocol, not re-randomized within a pair. This makes the paired difference a common-random-numbers (variance-reduced) comparison, which is what the paired hierarchical bootstrap in §G/Slot 7 assumes.
...
| `tau_cmd` | `[T,2]` | N·m | controller-requested joint torque before actuator limiting (also an observed channel, §C) |
...
| `temperature_true` | `[T,4]` | °C | privileged temperature at the four gauge stations; drives FBG thermal apparent-strain in §C but is **not** a deployable channel |
...
| `control_effort` | `[T,2]` | N·m | limited actuator-side effort after saturation but before actuator-gain loss; source for effort metrics and the nominal current proxy |
...
- **`current_proxy_obs`** = nominal-motor-constant × noisy measured motor current — an estimate of `control_effort`, taken **upstream** of the actuator-gain loss. `tau_cmd` is the controller's pre-limit request; `control_effort` is the saturated actuator-side effort; `tau_delivered_true` is the post-fault physical torque. Under an actuator fault the proxy stays ≈ nominal while `tau_delivered_true` drops, so C1 has **no direct torque-drop signal**; the drop reaches C1 only through *motion* (encoder/IMU), which is confoundable with a stiffness or load change. Distinguishing it is exactly S's job to test. Keeping true delivered torque out of every deployable suite is what keeps actuator attribution non-trivial (Slot 1, Slot 5).
...
- **Masks & missingness. [C4]** The on-disk observation array carries the full registry width; channels a suite does not have are written as **`NaN`** and marked off in the static `suite_available_mask`. A separate per-time, per-channel **`valid_mask`** carries dropout/validity; per-time, per-channel `measurement_time_s`, `availability_time_s`, and **`latency_age_s`** preserve asynchronous/latent delivery. Any model-side fill value is applied **only by the loader** and always travels with its masks; raw storage never silently imputes. Because C1 and S are *separate files* (separate `run_id`), a C1 file contains only `NaN` in the gauge slots — never hidden gauge values to peek at.
...
- **[C4] Leakage test (required, must exist before confirmatory generation).** An automated test constructs the **deployable feature loader for C0/C1/S using only that suite's observation index/root** and asserts it cannot import or resolve (a) the identity manifest or any provenance field such as `fault_setting_id`, (b) any privileged plant payload/index, (c) any label payload/index, or (d) another suite's observation payloads. It also checks that unavailable registry slots are `NaN` and masked. The test **fails the build** if any forbidden role/path/field is reachable. Supervised training joins labels to features in a separate allowlisted training-data builder and passes them as targets, never as deployable feature columns. This turns "no leakage" from a convention (Slot 12 method-failure guard) into a checked property.
- **Causality.** Windows are **past-only**. Availability metadata is channel-level; for any decision at `decision_time`, every value used has `availability_time_s ≤ decision_time`. `qd_obs` is derived causally from `q_obs` (no centered/future differences).
...
- **Formats (dependency-light):** `manifest.csv` (stdlib `csv`) is the path-free but **non-deployable** identity/split table from §A. Each persisted role has its own root and `index.csv`: `plant/`, `observations/<suite>/`, `labels/`, `estimator_outputs/<suite>/`, and `controller_logs/<suite>/`. Each role index contains `run_id, schema_version, config_hash, npz_path, sha256`; an observation index additionally carries the safe `split` assignment so its loader never needs the identity manifest. Persist **one numeric, non-pickled `.npz` per rollout per applicable role** (`allow_pickle=False`), rather than combining roles in one file. Immutable `schema.json` declares every field's shape, dtype, unit, role, and availability rule; immutable `config.json` contains all frozen values. A deployable loader receives only one `observations/<suite>/index.csv` and root. The identity manifest, labels, and privileged indexes are supplied only to their allowlisted builders/evaluators; the training-data builder may join labels as targets without exposing their fields to the feature tensor. **No Parquet or YAML dependency** for the interface unless a measured need justifies a reviewed amendment.
- **Paths** are project-relative. Machine-specific input roots are `argparse` arguments with `required=True`; output roots default under the packet/run tree and may be overridden explicitly. No absolute path is stored in a manifest or index.
