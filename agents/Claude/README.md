...
- **[`Reproducibility Packet/scripts/`](../../Reproducibility%20Packet/scripts) — sensor lane (mine, Session 6):** the sensor-realism + fault-injection model — `utils/schema_types.py`, `utils/sensor_model.py`, `utils/rng.py`, `utils/synthetic_plant.py`, `scripts/make_synthetic_plant_trace.py`, `scripts/run_sensor_model.py`, and `tests/test_sensor_model.py`. Built against schema v1.0; 18/18 packet tests green. The `PlantStepState`/`PrivilegedRecord` structs in `schema_types.py` are a **proposed shared interface** pending Codex's review. Codex owns the plant-side spike code in the same folder (`run_feasibility_spike.py`); both lanes share `utils/` and the Reproducibility Packet.
- **Chats with Codex** (co-owned, append-only): [`Phase 0 Coordination/`](../../chats/Claude-Codex/Phase%200%20Coordination) — **Concluded** Session 2; [`Claim Sheet Review and Division of Labor/`](../../chats/Claude-Codex/Claim%20Sheet%20Review%20and%20Division%20of%20Labor) — **Concluded Session 6** (all Phase-1 loops closed; see its `Summary.md`), concluded at the director's instruction; and [`Phase 2 Integration and Config Freeze/`](../../chats/Claude-Codex/Phase%202%20Integration%20and%20Config%20Freeze) — **Active**, opened Session 6, carries the current plant↔sensor integration + `config.json` freeze coordination.
- **Chat with the director** (co-owned, append-only): [`chats/Claude-Codex-Human/Chat Appends/`](../../chats/Claude-Codex-Human/Chat%20Appends) — **Active**; the director flagged Codex's chat-misplacement occurrence (Session 6). I acknowledged; it awaits Codex's acknowledgment before it can be concluded.
