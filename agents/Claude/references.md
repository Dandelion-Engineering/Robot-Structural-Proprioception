...
## Phase 2 — Two-layer evaluation harness: metrics + stats core (Session 7)

*One new source (Guo et al. 2017, below) plus the source-to-implementation mapping for the evaluation core. Most of this metric family was already grounded in this ledger during Phase 1 (Traub et al. 2024 for the calibration/selective/OOD separation; the scikit-learn and SciPy tooling docs), so the entries below record which existing source justifies each implemented function rather than re-citing. Implementation: `Reproducibility Packet/scripts/utils/metrics.py`, `utils/stats.py`, tests `tests/test_metrics.py`, `tests/test_stats.py`.*

**Guo, C., Pleiss, G., Sun, Y. & Weinberger, K. Q. 2017 — "On Calibration of Modern Neural Networks." *ICML 34*, PMLR 70:1321–1330.** https://proceedings.mlr.press/v70/guo17a.html
Defines the **Expected Calibration Error (ECE)** as the sample-weighted mean gap between confidence and accuracy across equal-width confidence bins, and documents that modern high-capacity classifiers are systematically over-confident (miscalibrated) even when accurate. *Informs:* the exact `expected_calibration_error` implementation (top-confidence binning, `n_bins=15` default, weighted |accuracy − confidence| gap). This is why the Claim Sheet reports calibration **separately** from the headline macro-F1 — an attribution head can be accurate yet over-confident, and an over-confident classifier that also abstains is a safety concern for the recovery controller. New to this ledger this session; canonical ECE reference.

- **Four-way macro-F1 with abstention-as-error.** `macro_f1` via `sklearn.metrics.f1_score(..., labels=[0,1,2,3], average='macro', zero_division=0)`, with abstentions encoded as an out-of-label sentinel so a declined known-class sample is a false negative for its class and never a false positive. Grounds: the **scikit-learn f1/macro** doc (Phase-1 entry) for the metric; **Traub et al. 2024** (Phase-1 entry) for *why* known-class abstention is scored as a headline error rather than excused.
- **Per-class recall + non-inferiority.** `per_class_recall` via `sklearn.metrics.recall_score(..., average=None)`; the −0.02 per-source-class lower-bound guard is applied by `stats.BootstrapResult.non_inferior`. Grounds: same scikit-learn tooling; the −0.02 margin is the Claim Sheet's pre-declared value.
- **Calibration (Brier / multiclass NLL / ECE).** Brier and NLL are direct NumPy renderings of their standard definitions; ECE follows **Guo et al. 2017** (above). Grounds: the **scikit-learn calibration** doc (Phase-1 entry) for the reliability-diagram framing.
- **Selective prediction (risk–coverage, false-abstention).** `risk_coverage_curve` / `selective_risk_at_coverage` sweep a confidence-acceptance threshold; `false_abstention_rate` measures in-distribution abstention. Grounds: **Traub et al. 2024** (Phase-1 entry) — risk/coverage working points and the calibration-vs-selective separation.
- **OOD detection (AUROC / AUPRC / false-accept @ ID-acceptance).** `ood_auroc` / `ood_auprc` via `sklearn.metrics.roc_auc_score` / `average_precision_score`; `false_accept_at_id_acceptance` is a biometric-style operating-point metric (threshold set to accept a target fraction of in-distribution samples, then measure OOD false-accept). Grounds: scikit-learn ranking-metric tooling; the operating-point convention is documented in-code and is a config-freeze-time decision.
- **`J_5s` tracking integral.** `j_5s` implements schema §G exactly: planar (x,y) L2 endpoint error of the **true deformed tip**, trapezoidal (`numpy.trapezoid`) over the control-grid samples in `[t_c, t_c+5 s]`, units m·s; `tracking_reduction_pct` is the S-vs-C1 percentage reduction. Grounds: schema §G / Claim Sheet Slot 7 (internal contract, not an external source).
- **Paired hierarchical bootstrap.** `stats.hierarchical_bootstrap_ci` resamples `pair_id` clusters then `train_seed` subunits within each, preserving C1/S pairing, and returns a percentile interval + the "excludes zero" decision; NumPy-only (percentile primitive), no SciPy import. Grounds: the **SciPy bootstrap** doc (Phase-1 entry) as the concept reference for percentile bootstrap CIs; the nested/paired resampling structure is the project's own (Claim Sheet Slot 7 / §A pairing). Bibliography note: if a formal citation is wanted for the hierarchical/cluster bootstrap at reconciliation, the standard target is Davison & Hinkley 1997 (*Bootstrap Methods and their Application*) — to be added only if actually cited in the Technical Report.

---

## Phase 2 — Diagnosis-estimator lane + eval corrections (Session 8)

*No new external sources this session; the estimator lane was built from anchors already in this ledger. Recording the source-to-implementation mapping while fresh, plus two forward corrections to the Session-7 eval entries (corrections propagate forward — the S7 entries above are left as the recorded turn). Implementation: `Reproducibility Packet/scripts/utils/estimator.py`, tests `tests/test_estimator.py`; metric addition in `utils/metrics.py`.*

- **Past-only windowed front-end + matched suite ablation.** `WindowFeatureExtractor` emits a fixed `[W, D]` tensor and summary-feature vector over the full channel registry for every suite, so the estimator is held constant while the sensor suite varies (C0⊂C1⊂S). Grounds: **Wensing–Niemeyer–Slotine 2024** (§4, load-bearing) — the identifiability nullspace of a conventional suite is *why* the matched comparison is the honest test; **Farrar & Worden 2007** / **Worden et al. 2007** (§5) — SHM as the feature-extraction → statistical-model pipeline, and the axiom that damage assessment requires comparison of two states (the front-end's healthy-reference standardization is exactly that).
- **Detection + calibrated-abstention rung (`WindowNoveltyDetector`).** A top-k sparse-change statistic standardized against a leave-one-out healthy reference, with a persistence-latched `detection_time_s` and an honest healthy-vs-not simplex that abstains on the fault *type* until a trained head exists. Grounds: **Farrar & Worden 2007** / **Worden et al. 2007** (§5) — novelty detection against a baseline as the core SHM primitive; **Zhou et al. 2021/2023** (§7, load-bearing for Slot 3) — uncertainty-informed diagnosis with OOD flagging, the basis for reporting an `unknown_score`/abstention rather than forcing a class; **Traub et al. 2024** and **Guo et al. 2017** (Phase-1/S7 entries) — why calibration/selective/abstention are first-class and known-class abstention is a headline error.
- **Specified-not-built learned rungs (temporal-attribution net, RMA latent) + `W`/stride.** The `[W, D]` front-end is the shared substrate for the matched learned temporal encoder and the RMA-style latent; both are trained post-config-freeze (they need confirmatory data). Grounds: **Kumar et al. 2021 (RMA)** (§3) — the adaptation-latent-from-proprioceptive-history baseline the structural suite must beat/complement; **Thuruthel et al. 2019** (§6) — embedded-sensor + temporal (RNN) proprioception, the template for the temporal encoder; **Lee et al. 2020 (CaDM)** (§4) — a context latent inferred from a short past window, the RMA-latent analogue. `W=512`/`stride=8` proposed from the 0.8 Hz diagnostic period vs. detection-latency tradeoff (mechanics-grounded; internal, not an external source).
- **[CORRECTION, forward] OOD operating point.** The S7 entry above (line ~313) described `false_accept_at_id_acceptance` at 95% *ID acceptance*. That was wrong against the contract: **Claim Sheet line 118** fixes false acceptance at **95% unknown-detection sensitivity**. Codex corrected the implementation (S7) to `unknown_threshold_at_sensitivity` on validation OOD → frozen threshold → `ood_false_acceptance_rate` on held-out OOD; I verified the correction against the contract and approved it this session. Grounds: Claim Sheet Slot 7 / line 118 (internal contract).
- **[ADDITION, forward] `coverage_at_risk`.** Claim Sheet line 118 also pre-registers "coverage at a pre-registered 5% selective-error ceiling," the dual of `selective_risk_at_coverage`, which the S7 metrics module did not render. Added `coverage_at_risk(...)` (max coverage whose selective risk ≤ ceiling) with a hand-computed test. Grounds: **Traub et al. 2024** (Phase-1 entry) for the risk/coverage working-point framing; Claim Sheet Slot 7 / line 118 for the pre-registered ceiling.

---

## Phase 2 — Synchronous-detection noise-floor analysis (Session 9)

*No new external sources were read this session; the analysis reuses the FBG noise-model sources already in §5 (Barrias 2016, Silveira 2021) and a standard signal-processing technique (synchronous/lock-in detection). Recording the mapping while fresh. Implementation: `Reproducibility Packet/scripts/analyze_synchronous_detection_floor.py`, tests `tests/test_synchronous_detection_floor.py`, artifact `results/synchronous_detection_floor/`.*

- **Detector-referred noise floor at the diagnostic frequency.** The bounded-burst mechanics gate (Codex, S8) screens the clean differential strain RMS against a per-sample credibility floor `max(gauge_resolution=1 µε, thermal_cross_sensitivity=10 µε/°C)`. That floor is a broadband, DC-scale quantity dominated by the thermal cross-sensitivity coefficient. But the differential signature is at the known 0.8 Hz probe frequency, and the deployable estimator reads a `W`-sample window, so the operative floor is the estimator's noise-equivalent strain (NES) at 0.8 Hz. Grounds for the noise magnitudes: **Silveira et al. 2021** (~10 µε/°C FBG thermal cross-sensitivity — the dominant low-frequency term the synchronous detector rejects) and **Barrias et al. 2016** (~1 µε FBG resolution — the white term that integrates down over the window), both §5. The analysis drives the *real* `OnlineSensorSession._gauge` pathology stack (hysteresis + thermal ramp + bias + random-walk drift + white + quantization + dropout) rather than a reimplementation, so the floor reflects the implemented sensor model.
- **Result mapping.** Over the real gauge stack with an aggressive 3 °C/window thermal ramp: broadband RMS ≈ 17 µε (thermal-dominated, at/above the 10 µε gate floor) → synchronous NES ≈ 0.10 µε (≈ 103× below the gate floor); the bounded one-cycle differentials (8.18/7.84/12.33 µε) detect at 100% (z ≈ 150–305). Sub-finding: W=512 spans 0.82 of a 0.8 Hz cycle (lock-in gain 0.63), so the frozen `W` should cover ≥1 probe period (W=640 → gain 1.02, NES 0.074 µε). This is a **detection** floor only; attribution still needs the learned head. Informs the excitation/`W` co-design (Slot 5/9) and motivates a proposed synchronous feature for `WindowFeatureExtractor`.
- **Bibliography note (reconciliation TODO, not cited from memory).** Synchronous/lock-in detection and matched-filter SNR gain are standard signal processing; if the Technical Report cites the technique formally, add a verified canonical reference at Phase-2 reconciliation (candidate: Meade, *Lock-in Amplifiers: Principles and Applications*, 1983) **only if actually used in the report** — not added now to avoid a citation-from-memory.

---

## Phase 2 — Synchronous estimator feature + safety-regression eval (Session 10)

*No new external sources this session: I closed two review-cycle loops (the synchronous-floor artifact and schema Amendment A1) and built two increments from anchors already in this ledger. Recording the source-to-implementation mapping while fresh. Implementation: `Reproducibility Packet/scripts/utils/estimator.py` (synchronous feature), `utils/metrics.py` (safety gate); tests `tests/test_estimator.py`, `tests/test_metrics.py`.*

- **Synchronous (lock-in) feature in `WindowFeatureExtractor`.** Added a per-registry-column harmonic amplitude at the 0.8 Hz probe frequency, computed with the shared phase-invariant `utils/synchronous.py` regression on each channel's own measurement grid, gated to emit only when the valid samples span ≥ one full probe period. This is the interpretable realization of the ~100× detector-referred headroom the S9 floor analysis found. Grounds for *why the probe-band feature helps*: **Silveira et al. 2021** (10 µε/°C thermal cross-sensitivity — the low-frequency term the lock-in rejects) and **Barrias et al. 2016** (~1 µε FBG resolution — the white term that integrates down over the window), both §5; the harmonic-regression statistic itself is the same standard signal-processing technique flagged in the S9 bibliography note (same reconciliation TODO — not cited from memory). Coupled decision: `RECOMMENDED_WINDOW` moved 512→640 so the default window spans a full 0.8 Hz cycle (internal, mechanics-grounded).
- **"No safety regression" gate (`safety_incident_rate` / `safety_flag_rates` / `safety_regression_delta`).** Renders the Claim-Sheet Slot-7 control-side clause over the privileged-truth `safety_flag` (schema Amendment A1, approved this session): a paired per-run delta `rate(S) − rate(C1)` the crossed pair×seed bootstrap aggregates, with "no regression" = upper 95% bound ≤ 0. Grounds: **Claim Sheet Slot 7** and **schema Amendment A1** (internal contract); the crossed-bootstrap aggregation traces to the S7/S8 stats entries above.
