...
## S137 FIRST - STEP 4a IS CLOSED.  THE 4b BUILD IS SPLIT AND HALF ONE IS ON CODEX AT ROUND 1.
...
*** THE REVIEW METHOD IS THE DIRECTOR'S SUPERSEDING PROTOCOL, appended to
    ...
    THREE OWNER-REVIEWER ROUND-TRIPS and THE LIMIT NEVER FORCES APPROVAL, with five terminal
...
*** THE PROJECT STATE IN ONE LINE: EVERY SCIENTIFIC LANE IS STILL SPENT OR SHUT, SLOT-8 STEPS 1,
    2 AND 3 ARE CLOSED, SUB-STEP 4a IS NOW CLOSED TOO, AND THE ONLY OPEN LOOP IS THE 4b-i
    CONTRACT I BUILT IN S136, ON CODEX AT ROUND 1. ***
...
WHAT S136 DID, AND THE SCOPING DECISION IT TOOK.  *** READ THE NEW CARD AND THE NEW CHAT TAIL
BEFORE ANYTHING ELSE. ***
  I SPLIT THE 4b BUILD IN HALF.  Section 10 names 4b as ONE sub-step; it is a 21-row read order,
  14 exit codes, a dedicated coherent geometry fixture, an audit-hook observer and tests B1-B8.
  Under a three-round limit that cannot be accepted, rejected or returned as one candidate.
  ...
  *** SUB-STEP 4b DOES NOT CLOSE WHEN THE 4b-i CARD CLOSES.  It closes when BOTH halves are built
      and BOTH reviews are closed.  THE SPLIT MOVES NO GATE, PRECONDITION, INVARIANT, EXIT CODE OR
      AUTHORIZATION - it is a review-scoping decision, and the boundary is the design's own: 4.1
      calls rows 1 and 2 "the first boundary" and row 3 completes it.  I ASKED CODEX TO RULE ON THE
      SPLIT BEFORE REVIEWING THE CONTENTS.  Lesson 224. ***

THE ONE OPEN LOOP - TWO NEW FILES, NO TRACKED FILE MODIFIED, APPROVED BY ME IN MY S136 TURN.
  ...
  Reproducibility Packet/tests/test_connection_record.py
    ...
    50,022 B / 1,245 LF / 0 CR / pure ASCII / no BOM / final newline / 212 tests
  *** THOSE RAW DIGESTS ARE OF THE **BLOB** BYTES.  These are `*.py`, CODEX'S S128 RULING THAT NO
  ...
  WHAT THE CONTRACT IS: step 1 `authenticate_record_bytes` (hash before parse); step 2
  `parse_connection_record` (strict parse + the WHOLE 3.2 field table, no optional field, no
  default, NO TOLERATED EXTRA KEY at any level - which is also what enforces W12); step 3
  `bind_root_domains` (THE PACKET ROOT IS AN EXPLICIT PARAMETER and that ONE root governs the
  schema, the config, all six source artifacts AND the 4.7 output parent together - the W8 seam);
  and `expected_open_set` (the EXPECTED side of W3; the OBSERVED side is 4b-ii's audit hook).
  *** WHERE FACTS ARE POINTED AT RATHER THAN COPIED: refusals raise `verification_scene`'s OWN
      `VerificationSceneError` with ITS codes - THIS BUILD ADDS NO ERROR TYPE AND NO CODE; the 20
      schema-A field names and the five integer ones are DERIVED from
      `storage_contract.IdentityManifestRow`; the digest predicate is `re_full_sha256`; canonical
      JSON is `protocol_p.canonical_json`.  TESTS ASSERT EQUALITY against those owners. ***

THREE DECISIONS I TOOK THAT THE DESIGN LEFT TO THE BUILD.  ALL THREE ARE FLAGGED AS CONTESTABLE IN
THE CHAT AND ARE CHEAP TO MOVE NOW, EXPENSIVE AFTER 4b-ii IS WRITTEN AGAINST THEM:
...
THE MEASUREMENT THAT WENT AGAINST ME IN S136, AND IT IS THE ONE TO CARRY (lesson 223):
  ALL 209 TESTS PASSED FIRST RUN.  The mandatory two-pass mutation control (44 mutants, scratch
  dir outside the repo) REPORTED FIVE SURVIVORS AND FOUR WERE MY OWN TESTS PASSING FOR THE WRONG
  REASON.  *** `"newline" in str(error)` IS SATISFIED BY THE CANONICAL ROUND-TRIP'S OWN SENTENCE
  *"no BOM, no trailing newline"*, so DELETING THE NEWLINE RULE LEFT THE SUITE GREEN.  Same shape
  on the rooted-path, split-membership and digest-form branches: EACH IS SUBSUMED BY A LATER CHECK
  WHOSE MESSAGE CONTAINS THE PHRASE THE ASSERTION LOOKED FOR.  A UNIQUE PHRASE IS NOT ENOUGH - IT
  MUST NOT APPEAR AT A LATER SITE THAT ALSO REFUSES THE SAME INPUT, AND ONLY THE SWEEP FINDS
  THAT. ***  The fifth, `_resolve_under`'s containment guard, is UNREACHABLE from a well-formed
  record and is now held by a DIRECT UNIT TEST - a guard no mutation can break is a guard nothing
  checks.  AFTER THE FIXES: 42/42 real mutants caught, BOTH negative controls surviving, identical
  across both passes, target digest restored equal.
...
THE ARTIFACT SHAPES AND THE AUDIT INSTRUMENTS ARE IN `agents/Claude/Permanent Instruments.md`.
  ...
  its sections answers which question.  *** STANDING LESSONS NOW REACH 224 AND ALL OF THEM LIVE
...
- **Running packet tests:** from the REPO ROOT, `./venv/Scripts/python.exe -m pytest -q "Reproducibility Packet/tests"`. **Scope pytest to that path** - a root-wide invocation collides on duplicate test module names in the ignored `tmp/session6_packet_copy/`. *** THE FULL-SUITE FIGURE IS **2,479** (S136, 192.86 s, 0 failed, 0 collection errors) - it was 2,267 and the S136 4b-i build added 212.  It is CLEAN: the Smart App Control block that made S128 and S129 unmeasurable is OVER - see the head block and `director_requests.md` entry 2. **DO NOT QUOTE 1,328 OR 1,344; THEY ARE ARTIFACTS OF A BROKEN ENVIRONMENT.** *** The two Slot-8 files run **159 passed in 31.70 s**, and 159 again under `python -O`. Earlier clean history: 2,267 (S130, 221.38 s; S131, 204.35 s), 2,108 (S118, 126.88 s), 2,005 (S115-S117), 1,863 (S114), 1,861 (S113), 1,792 (S102), 1,753 (S93), 1,551 (S85-S90), 1,370 (S77), 1,306 (S72), 595 (pre-S51 baseline). *** MY S115 REPORT SAID 2,004 AND THAT WAS A TYPO IN MY OWN ARITHMETIC: 1,863 + 142 = 2,005; Codex corrected it forward and I MEASURED the correction in S116 rather than conceding it. *** `test_payload_boundary_extension.py` collects **170**; the two closed Step-2 seam files together collect 124; `test_dev_fit_analysis.py` collects 35; **`test_connection_record.py` collects 212** (3.82 s, and 212 under `python -O`). **Set `PYTHONIOENCODING=utf-8` for anything that prints non-ASCII** - the console is cp1252. **Use ASCII in probe scripts and in anything a gate prints.** **Set `MPLBACKEND=Agg` for anything that renders a figure outside pytest** - the Slot-8 test files call `matplotlib.use('Agg')` at import, but a bare probe script or the fixture CLI will pick up the interactive backend, and the CLI's fixture mode ends by opening the menu.
