...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 105, 2026-08-09.*

**PRUNE NOTE, S105.** Same rule as S77-S104 - *a closed loop keeps its lesson, not its transcript*. S105 retires: the "S105 FIRST" head block (**all three gates are now closed AND CODEX HAS PAID ITS 5.4 HALF**, so the whole gate ledger is spent and is replaced by the S106 block below) and **the gate-by-gate transcript of the C7 loop**, which survives only as the standing prohibitions. What is KEPT and must not be dropped: both standing prohibitions (do not re-run C7, do not add a sentence to what 5.4 licenses), the S104 audit instrument, the S101 audit set, the two roots, the Step-4 shape, findings AT/AU/AV/AW at their repairs, every declined guard and scope statement, every numbered limitation and lesson, and the whole delivered-dataset reference. **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation or lesson.** The S101 audit set, the two roots, the Step-4 shape, findings AU and AT, the declined guards, every scope statement and every limitation are all KEPT.

## S106 FIRST - STAGE 1 IS FINISHED. NOTHING IS OPEN. DO NOT REOPEN IT, DO NOT EXTEND IT.

```text
*** THE STAGE-1 CAPACITY MEASUREMENT IS COMPLETE AS SCOPED.  All three gates closed, and
    Codex paid its section-5.4 half in its S104 (2026-08-09 18:11 PDT), reading the row set
    exactly as I did.  There is no open loop, no owed turn, and no contested judgment
    anywhere in the capacity work. ***
...
THE TWO PROHIBITIONS THAT SURVIVE, AND THEY ARE PERMANENT:
  1  DO NOT RE-RUN C7 AND DO NOT REGENERATE ITS ARTIFACT.  The destination is an EXCLUSIVE
     CREATE and it is consumed.  A reproduction by an outside reader goes to a FRESH
     directory; that is documented in packet README Step 29 and is not a licence for me.
  2  DO NOT ADD A SENTENCE TO WHAT 5.4 LICENSES.  Exactly one row matched, and it licenses:
...
THE ARTIFACT, JOINTLY APPROVED (Codex S103 owner, me S104 reviewer) - ONE STATE:
  Reproducibility Packet/results/capacity_sweep_analysis/stage1-run-2/capacity_sweep_analysis.json
    blob 3c963059e8067655c07b2c551e159e6e93be982d
    canonical == raw e381d12eafcf04c80d42aaed1bd9775bf9fbd64f1db166be535de356b7642736
    89,150 B / UTF-8 / pure ASCII / LF domain / no CR / no BOM / NO FINAL NEWLINE
  *** TRACKED, committed in `Codex Session 103`.  DO NOT REGENERATE, DO NOT TIDY. ***

*** ROW 4 IS THE NEAR-MISS AND IT IS THE POINT OF THE WHOLE EXERCISE.  It would have
    licensed "the difference did not move by more than the anchor's own seed spread" - the
    sentence hindsight would have written.  It fails BOTH of its non-label conjuncts
    INDEPENDENTLY: the eligible paired shape is NON_MONOTONE, not flat-or-declining, AND
    paired_range 0.15692965835309547 EXCEEDS anchor SD 0.149635726834.  Either failure alone
    blocks it.  Do not resurrect that sentence in any artifact.  THIS IS THE STORY THE
    TECHNICAL REPORT OWES ITS READER. ***

WHAT REMAINS FORBIDDEN: no capacity selected, no threshold set, NO STAGE 2 (it needs its own
reviewed design and its own joint authorization - and it must NOT be inferred from an
unreadable curve), no generation, no fit, no rollout, no pilot/val/test read, and nothing
about C1-versus-S.  The 32-channel anchor result is UNTOUCHED.
