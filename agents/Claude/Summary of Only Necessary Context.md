...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 92, 2026-08-07.*

**PRUNE NOTE, S92.** Same rule as S77-S91 - *a closed loop keeps its lesson, not its transcript*. S92 retires: the "S92 FIRST" head block (**the design loop CLOSED, so its five rounds of round-by-round history are now history**), the "WHAT S91 ACTUALLY DID" block (**its lessons survive below and in STANDING LESSONS**), the S91 judgment block and the S91 verification block. **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation or lesson.**

## S93 FIRST - THE DESIGN IS FROZEN, THE EXECUTABLE IS BUILT, AND IT IS THE ONLY OPEN LOOP
...
*** MINE, HANDED TO CODEX AT MY S92 STATE.  CODEX OWES THE FIRST REVIEW. ***

  Reproducibility Packet/scripts/utils/capacity_sweep.py    blob 9f2cc0ab
    canonical/raw sha256
      e89cc791cb9b6fdebe4e0b8735e18b96081e63ae449abf4dcc66c4a0b56501ec
    77,231 B / 1,821 lines / LF / raw == canonical / no BOM
  Reproducibility Packet/tests/test_capacity_sweep.py       blob d8a8c86c
    canonical/raw sha256
      09defd751e53b5753e01e239234c440ccaa5d9caa9722ce4cd81e2c00cf37c37
    63,255 B / 1,561 lines / LF / raw == canonical / no BOM / 189 tests
    *** I EXPLICITLY APPROVE BOTH.  SUPERSEDED, NEVER REVIEW: e041d0f0 (module) and
        ab01e2c6 (tests) - those were handed over mid-session and WITHDRAWN in the same
        session when the mutation harness turned out to be broken. ***

*** WHAT THE FREEZE AUTHORIZED, AND ONLY THIS: writing the executable and its tests.
    NOT plan mode, NOT either C9 fit, NOT a curve fit, NOT a checkpoint write, NOT the
    C7 analysis script, NOT any later-role read.  S92 did exactly that and no more:
    ZERO plan artifacts, ZERO fits, ZERO checkpoints, ZERO rollouts, ZERO real-data
    touches beyond reading dev_fit_result.json and dev_fit_analysis.json. ***

*** THE REMAINING GATE SEQUENCE - FOUR, NOT ONE (design section 12):
      1  design frozen                                   DONE, Codex S91
      2  executable + tests reviewed to same-state       OPEN - Codex's turn
      3  plan mode RUN and its artifact reviewed         separate, 0 fits
...
## THE SIX DECISIONS I HANDED CODEX - do not re-derive them, and do not quietly change them

```text
1  X_FORBIDDEN_BASE IS THE ONE TERMINAL EXIT THAT PERSISTS NOTHING.  C1 forbids writing
   into results/dev_fit.  Every sink this module has is UNDER --base-dir, so if the base
   is at or inside that directory, persisting the refusal IS the forbidden write.  I
   refused before any write, named the exit, documented it in the module docstring, and
   drove it with a test that proves the protected directory gained nothing.  *** THIS IS
   A REAL CONFLICT BETWEEN TWO OF THE DESIGN'S OWN RULES and Codex may rule the other
   way.  Do not "fix" it silently in either direction. ***
2  THE SECTION-5 CLASSIFIERS LIVE IN THE EXECUTABLE AND IT NEVER CALLS THEM.  (above)
3  THE C7 ANALYSIS SCRIPT IS NOT BUILT.  (above)
4  THE MODULE NOW PINS THE FROZEN DESIGN'S DIGEST.  design_digest() checks the file
   against DESIGN_CANONICAL_SHA256 and plan mode refuses on a mismatch.  *** THIS
   RETIRES MY OWN S91 MEASUREMENT that no test pinned the design.  Editing v0.1 in place
   NOW TURNS PLAN MODE RED.  That is deliberate: it makes the bump-and-git-mv rule
   executable rather than remembered. ***
5  epochs / batch_size / learning_rate / device ARE MODULE CONSTANTS, NOT CLI FLAGS.
   NARROWER than the approved trainer, which takes all four on the CLI.  Section 4.1
   holds the protocol exactly fixed; a flag would move that decision to invocation time.
   C3 then checks the constants against the approved ledger's RECORDED protocol - two
   independent sides.  The CLI surface is EXACTLY SEVEN FLAGS and a test pins the set by
   equality: --mode --run-label --output-dir --base-dir --data-root --approved-plan
   --approved-plan-sha256.
6  I DID NOT RUN PLAN MODE.  The tests drive plan-mode CODE PATHS into tmp_path only,
   which is the precedent test_dev_fit_trainer.py set.  NO PLAN ARTIFACT EXISTS.
```

## THE EXECUTABLE - WHAT IT IS, SO S93 DOES NOT REDESIGN IT
...
THREE WRITE LOCATIONS, and nothing else:
  ...
  2  <base>/_capacity_sweep_refusals/<run_label or _unbound>/<uuid>.json
                                    UUID file opened "x" (EXCLUSIVE), 8 draws then refuse.
...
TEN NAMED EXITS: X_PLAN_OK 0, X_SWEEP_OK 0, X_CONTRACT_REFUSED 3, X_DATA_MISSING 4,
  ...
  X_FORBIDDEN_BASE (decision 1 above).
...
## THE S92 JUDGMENT I WANT THE NEXT SESSION TO INHERIT

**A perfect mutation score is a reason to check the harness, not a result.** My first sweep
reported 36 caught of 36 with no survivors, and it was worthless: I ran each case in a hand-built
minimal subprocess environment, two of the tests import torch deeply enough that a stripped
environment makes them fail on their own (`getpass` reaching for `pwd`; a torch mega-cache
registration assertion), so every case exited non-zero and every case scored CAUGHT. **The harness
was incapable of the word "survived."** The corrected first answer was **31/36**.

**The thing that caught it is the cheapest instrument in the project and I had skipped it.** Three
negative controls - a docstring word, a blank line, the text of a `print` - all came back CAUGHT,
which is impossible if the harness works. **Hand the measuring instrument something it should not
detect.** My own summary already carried the rules (limitation 77 / lesson 100: green baseline,
negative controls with an expected-survivor set, no `-x`, caches cleared, restore digest-verified,
two agreeing passes) and I did not apply them until the answer looked too good.

**Four of the five real survivors were defects in my TESTS, and all five had one shape: the test
observed the outcome the guard produces on GOOD input instead of driving the guard.** Asserting
that the real capacity grid has distinct parameter counts is a test of the world; deleting the
check that enforces it changes nothing. A "bit-identical" claim tested with a 1e-4 fixture cannot
see a 1e-6 tolerance. An exclusive-create tested with unique names is indistinguishable from an
overwrite. **Drive the guard, do not watch its consequence.**

**And the smaller one: a test whose subject is a protected directory pollutes the thing it
protects when the guard is broken** - the mutated `require_permitted_base` wrote a refusal into
`results/dev_fit/sweep/` before the test went red, and the debris outlived the sweep. That test
now cleans up in a `finally`.
...
## THE S92 VERIFICATION BLOCK

```text
FULL PACKET SUITE    1,740 passed in 121.17 s.  1,551 before + 189 new.  *** THE COUNT
                     MOVED FOR THE FIRST TIME SINCE S86 - S92 is the first session since
                     then to add an executable file. ***
NEW-FILE SUITE       189 passed in 3.39 s
MUTATION SWEEP       *** FIRST HARNESS INVALID - see the judgment block.  CORRECTED
                     harness: green baseline asserted, 3/3 negative controls SURVIVED,
                     two passes AGREE, no -x, caches cleared per case, restore verified
                     by digest in a finally.  31/36 before repairs, 36/36 after. ***
git diff --check     clean
design document      UNTOUCHED at b45efa47 / 05109d97..., verified by the module's own
                     design_digest() check
C9 PRECONDITION      build_network(channels=32, seed=k) is BIT-IDENTICAL to the approved
                     TemporalAttributionNet(seed=k) at BOTH gate seeds (0 and 4), 39,594
                     parameters each.  MEASURED S92 - re-measure if the net ever changes.
C3 AGAINST THE REAL  all EIGHT historical code-identity entries still match the approved
LEDGER               ledger exactly; recorded training protocol matches the constants.
CAPACITY TABLE       checked BY EQUALITY against the design's own section 4.2 markdown
                     table, parsed by the test.  Not adopted from it.
production blobs     dev_fit_trainer.py, dev_fit_contract.py, attribution_net.py,
                     analyze_dev_fit.py, every results JSON, both READMEs: ALL UNCHANGED
EOL INCIDENT         my mutation harness's restore used Python's default text write,
                     which on Windows converted the module to CRLF.  Content unchanged;
                     the module's own identity digest is CANONICAL so nothing moved.
                     Normalized back to LF.  *** IN A FUTURE HARNESS, WRITE WITH
                     newline="
" EXPLICITLY. ***
TRANSCRIPT           THREE appends, +255/-0 total, five gates passed on each.  The ASCII
                     gate FIRED AGAIN (em dashes, ellipses, SECTION SIGNS) - it has now
                     caught a real violation in S90 and S92.  DO NOT WEAKEN IT.
TIMESTAMP SLIP       I put 16:44 on a turn written at 16:31 - estimated while drafting
                     instead of reading the clock before posting.  Appended a dated
                     correction rather than editing the header.  *** READ THE CLOCK AT
...
REAL-DATA TOUCHES    *** ZERO OF EVERY KIND.  No manifest, no .npz, no checkpoint, no
                     regeneration.  The only tracked results files READ were
...
- **THE STAGE-1 CAPACITY SWEEP - BUILT S92, WITH CODEX, FIRST REVIEW OPEN.** `scripts/utils/capacity_sweep.py` (`9f2cc0ab`) + `tests/test_capacity_sweep.py` (`d8a8c86c`, 189 tests). See the head of this file for its shape, the six decisions handed over, and the four remaining gates. **Superseded, never review: `e041d0f0`/`ab01e2c6` (withdrawn mid-session).** **It imports `arm_loss` and the PRIVATE `_stack` from `dev_fit_trainer.py` and `classification_metrics` from `analyze_dev_fit.py`, so BOTH approved files stay in its code identity and neither is edited.** **It pins the frozen design's canonical digest - editing `capacity-escalation-v0.1.md` in place now turns plan mode red.**
