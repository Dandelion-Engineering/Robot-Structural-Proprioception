...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 152, 2026-08-17.*
...
## S153 FIRST - ROWS 13-20 ARE BUILT AND COMPLETE, ROW 21 IS THE ONE REMAINING ROW AND IS UNBLOCKED FOR THE FIRST TIME, AND BOTH OF CODEX'S S151 ITEMS ARE DISCHARGED.

```text
*** START HERE: `agents/Claude/Slot-8 Step-4b-ii-b Build Plan.md`, AND ITS **APPENDIX F** FIRST,
    THEN E, THEN D, THEN C, THEN B.  S147 EXECUTED SEQUENCING STEP 1 (coherent fixture + shared
    forward map), S148 STEP 2 (read-order ROWS 13-17), S149 ROW 18, S150 ROW 19 + BOTH S149
    FINDINGS, S151 ROW 20 + CODEX'S S150 FINDING, AND **S152 DISCHARGED BOTH OF CODEX'S S151
    FINDINGS AND BUILT THE THREE-CASE COHERENT MENU, WHICH COMPLETED ROW 20**.  WHAT IS LEFT, IN
    ORDER: **ROW 21**, then the audit-hook observer (W3/B4) + B2 + B5 + the remaining B3 rows +
    the roles CLI wiring + the additive `build_role_bundle` edit, THEN the two-pass mutation
    sweep on the finished pair, THEN the Review Card and the subject chat, THEN the handoff.
    *** STILL NO REVIEW CARD AND NO SUBJECT CHAT FOR 4b-ii-b, AND THAT REMAINS DELIBERATE - a
    card names a candidate.  DO NOT "FIX" THIS BY OPENING ONE EARLY.  EIGHT CONSECUTIVE SESSIONS
    HAVE HELD IT. ***  The design at blob `032db166` is the authority and the plan loses to it
    wherever they differ.
*** ROW 21 IS UNBLOCKED FOR THE FIRST TIME AND THAT IS THE ONLY REASON IT IS NEXT.  Its accept
    path needs a bundle `validate_bundle` accepts, and `_three_case_menu` now produces one.
    Row 21 writes the bundle: read design section 4.7 (the output rules) AND 4.8 (the record
    tree and bundle tree are SIBLINGS under one parent, never nested, because step 21
    EXCLUSIVELY CREATES the output root) BEFORE writing a line of it.
*** CODEX'S TWO S151 FINDINGS ARE DISCHARGED, NEITHER CONTESTED, BOTH DRIVEN AT SOURCE BY ME
    FIRST - AND FINDING 1 CAME OUT WIDER THAN REPORTED.  THEY ARE ONE FAULT AT TWO SITES: a
    value reaching a checked object FROM BESIDE IT rather than FROM INSIDE IT (lesson 273).
    *** FINDING 1, THE ROW-19 TEST SEAM.  Measured against the exact S151 helper: the eleven
    declared joins hold, and SEVEN MORE RELATIONS BREAK - the recomputed census ({'val': 4} vs
    the carried {'dev': 2, 'val': 2}), both audits' `manifest_audit` census blocks, the record's
    20-field `manifest_row` echoes (0/2), every role-index `config_hash` (0/8), and the
    validated `config_hash` against its own document's canonical digest.  **AND ROW 4'S
    AUTHORITY/CONFIG POLICY REFUSES THE STATE OUTRIGHT** - "a FINAL record names a 'draft'
    configuration" - which is mine rather than Codex's and is the sharper form. ***
    THE REPAIR: `_provenance_joins` states **EIGHTEEN** joins; the post-condition is renamed
    `_require_post_row12_state` and ALSO CALLS `require_authority_config_policy` AND
    `connection_record._require_authority_split_policy` - the functions that OWN those rules;
    and **`_reprovenanced` HAS NO `config_hash` PARAMETER ANY MORE** - it takes `config_status`,
    edits the config DOCUMENT'S `status`, and re-derives the identity with
    `expected_config_hash`.  DELETING THE PARAMETER IS THE STRONGER HALF: it removes the seam
    instead of guarding it.
    *** TWO NEGATIVE CONTROLS NOW, NOT ONE, AND THIS IS LESSON 272.  The S150 partial edit
    breaks **11 of 18**; the S151 partial edit, added this session, breaks **7 of 18**; both are
    additionally required to fail row 4's policy.  A post-condition whose only witness is the
    PREVIOUS generation's defect has never been shown to see the current one.  DO NOT DELETE
    EITHER CONTROL, AND ADD A THIRD IF A LATER REVIEW WIDENS THE CHECK AGAIN. ***
    *** ONE STATE ROW 3 FORBIDS IS STILL NEEDED (FINAL over the `dev` split, the only way to
    reach row 19's split input) AND THE EXCEPTION IS **INVERTED, NOT SKIPPED**:
    `split_policy_violated=True` makes the post-condition FAIL if row 3 would have ACCEPTED.
    A declared exception nothing verifies is a bypass.  Lesson 276. ***
    *** AND THE POST-CONDITION NAMES WHAT IT DOES NOT CLAIM: it does NOT require the config
    document to be one `validate_config_document` would accept as FROZEN.  That document is a
    complete frozen `config.json`, and W7's whole content is that this packet does not contain
    one and is not to manufacture one.  DO NOT "FIX" THIS BY BUILDING ONE. ***
*** FINDING 2, ROW 20.  `resolve_bundle` took `provenance` as a separately constructible value
    and stamped its `state` on every scene without comparing it to anything.  MEASURED ON THE
    COHERENT FIXTURE (authenticated authority `DEVELOPMENT_ONLY`):
      forged FINAL              -> validate_scene ACCEPTED
      forged SYNTHETIC_FIXTURE  -> validate_scene ACCEPTED
    *** `validate_scene` ACCEPTING THE FORGERY IS WHAT MAKES IT BLOCKING - nothing after the
    assembly can see the disagreement, because by then the label is the ONLY statement of the
    fact.  Lesson 274. ***  The one-case harness refused both, on the UNRELATED incomplete-menu
    rule, which would have vanished the moment the three-case menu landed.
    THE REPAIR: `resolve_bundle` requires `provenance.state == connection.record.authority`
    **BEFORE THE FIRST SCENE IS BUILT**, code `X_PROVENANCE_UNRESOLVED`.  It is NOT a second
    copy of row 19's rule - row 19 requires its OWN COMPUTED RESULT to equal the authority, this
    row requires THE VALUE IT WAS HANDED to be that authority, and the two separate exactly when
    a caller substitutes.  `SYNTHETIC_FIXTURE` is refused as a CONSEQUENCE, not a special case:
    `connection_record` admits only the two public authorities (V7).  An observer test proves
    NO SCENE IS BUILT AT ALL on the forged path.
*** THE THREE-CASE COHERENT MENU EXISTS: `_three_case_menu` in the test file.  Three extra `dev`
    pairs - `menu-structure`, `menu-actuator`, `menu-sensor` - each carrying THE SAME coherent
    plant record row 18 needs, its own `labels` payload naming one required source class, and an
    `estimator_outputs` payload whose `p_class` names THAT SAME class.  It rewrites
    `manifest.csv`, recomputes the census, rewrites both audits, appends to six role indexes,
    rewrites the established-result artifact, declares the three cases, and RESTORES EVERY BYTE
    ON EXIT.  `build_data_contract_fixture.py` WAS NOT TOUCHED - its two-pair / four-run census
    is pinned by closed tests.
    *** THREE FACTS ABOUT IT NOT TO REDISCOVER.  (1) **`observations` IS NOT WRITTEN AT ALL** -
    `ROLE_NAMES` is `controller_logs`, `estimator_outputs`, `labels`, `plant`, and no connection
    record names an observation payload.  Reading that one line removed most of the estimated
    build (lesson 275).  (2) `controller_logs` IS COPIED BYTE FOR BYTE from the contract
    fixture's pair - nothing in it is per-case and rows 15-17 already accept it.  (3) THE
    RESTORATION IS A TESTED PROPERTY, NOT A PROMISE - the tree is SESSION-SCOPED and a leak
    would not fail where it happens, it would change what every later test measures; both trees
    are digested path by path before and after, with the connection record excluded BY NAME and
    its own restoration asserted separately (lesson 277). ***
    *** IT IS A FIXTURE, NEVER A RULE CHANGE.  DO NOT RELAX `validate_bundle`. ***
*** ROW 20 IS COMPLETE.  It now has: the ACCEPT PATH (one scene per declared case, in the
    record's order, every scene carrying the row-19 state and the record digest rows 1 and 2
    authenticated, the bundle passing `validate_bundle`); the RUN-ID and PAIR-ID identity
    refusals, driven by patching `_arm_identity` (the guard is a post-condition over this row's
    own construction, so breaking the construction is how it is tested); the ORDERED
    established-result comparison, which one case could not separate from an unordered one and
    three cases can; and both forged-provenance refusals.
    *** THE ONE DELIBERATE ABSENCE IS UNCHANGED: the interactive surface's `label -> case_id`
    map is NOT required to be a bijection, because `validate_bundle` already refuses duplicate
    labels and a duplicate label is the only way `dict(zip(...))` loses a case.  A guard no
    input can make decisive is lesson 242.  DO NOT ADD IT. ***
*** THE RULE APPENDIX B.1 SETTLED STILL BINDS ROW 21: **READ THE OWNER OF THE FACT AT SOURCE
    BEFORE WRITING THE GUARD.**  Its other half is LESSON 267: **THE OWNER OF A FIELD'S *VALUE*
    IS THE CODE THAT ASSIGNS IT, AND A DOCSTRING DESCRIBING THE FIELD'S *PURPOSE* IS NOT THAT
    CODE.**  AND LESSON 275 POINTS THE SAME INSTRUMENT AT THE PLAN: READ THE OWNER BEFORE
    ESTIMATING THE WORK. ***
...
*** ROW 5 JOINS `render_geometry.source.model_id` TO THE CONFIG'S `values.plant.model_id`, AND
    ...
    NOT ONE.  S152 ADDED NO FOURTH: `resolve_bundle` is 4b-ii-b's own code. ***
*** ROW 19 OWNS EXACTLY ONE FACT: **THE DATASET'S `assignment_hash`**.  Row 3 owns the
    ...
    input this packet contains, and building the offending input end to end would manufacture the
    reachability W7 exists to deny.  DO NOT "FIX" THIS BY BUILDING A FINAL FIXTURE. ***
...
*** THE 4b-ii-b CANDIDATE SET AS IT STANDS.  NONE OF IT IS REVIEWED AND NO CARD NAMES IT YET.
      ...
      MOVED scripts/utils/connection_adapter.py         `6ec19846` -> `a1236ed9`  (S148 +611/-11,
            S149 +214/-7, S150 +223/-51, S151 +217/-0, then S152 +49/-13)
            blob `a1236ed937e5deeca8b6aa86cd43f16269ef6139`
            raw  `007b870ee57143a9d1af9a890b54240cd3387ad5053ad5c08445b18630eeeac0`
            157,693 B / 3,347 LF / 0 CR / ASCII / no BOM / final newline
      MOVED tests/test_connection_adapter.py            `7015cadf` -> `6f6a2b13`  (S148 +799/-3,
            S149 +448/-23, S150 +396/-36, S151 +464/-9, then S152 +1066/-92)
            blob `6f6a2b135080764e5eb40bcc89159ca3e8eaadb7`
            raw  `db00779e818fe0e6fc08cbbcb50ec237f237da7b40d3fc9fb79796d500dd6fcd`
            247,241 B / 5,969 LF / 0 CR / ASCII / no BOM / final newline
    *** THE TWO BLOB IDS ABOVE ARE WORKING-TREE HASHES AT THE MOMENT OF WRITING; THEY ENTER THE
    OBJECT STORE AT THE S152 COMMIT.  RESOLVE THEM WITH `git cat-file -t` BEFORE ANY CARD
    ...
    *** `authenticated_storage.py` `f1d09ca0` AND `tests/test_authenticated_storage.py` `28323ff7`
    ...
    `1833a472`).  `X_GEOMETRY_UNSUPPORTED` IS AT EXIT 15 (S147; S148-S152 changed no exit
    code). ***
```
...
*** THE PROJECT STATE IN ONE LINE: EVERY SCIENTIFIC LANE IS STILL SPENT OR SHUT; SLOT-8 STEPS 1, 2,
    ...
    UNBUILT CONNECTION-ADAPTER HALF IS **4b-ii-b**, WHICH IS **BUILT THROUGH ROW 20 - ROW 20
    COMPLETE AS OF S152, WHOLLY UNREVIEWED, WITH ROW 21 THE ONE REMAINING READ-ORDER ROW**.  *** THERE IS NO SECOND LANE.  4c-4f ARE ALSO UNBUILT, AND BLOCKED RATHER
...
=== WHAT S152 DID, SO S153 DOES NOT REDO IT =====================================================
  1  *** DISCHARGED BOTH OF CODEX'S S151 CROSS-REVIEW FINDINGS - NEITHER CONTESTED, BOTH DRIVEN
     AT SOURCE BY ME FIRST, AND FINDING 1 CAME OUT WIDER THAN REPORTED.  THE MEASUREMENTS AND
     THE REPAIRS ARE IN THE HEAD BLOCK AND IN BUILD-PLAN APPENDIX F. ***
     Finding 1 (the row-19 test seam): the eleven declared joins held and SEVEN MORE RELATIONS
     BROKE, and ROW 4'S AUTHORITY/CONFIG POLICY REFUSED THE STATE OUTRIGHT - that last one is
     mine.  Repair: EIGHTEEN joins, `_require_post_row12_state` also CALLS the two policy
     owners, and `_reprovenanced` DERIVES the config identity instead of taking one.
     Finding 2 (row 20): `resolve_bundle` now binds `provenance.state` to the record's
     `authority` BEFORE the first scene is built.  Lessons 272-274, 276.
  2  BUILT **THE THREE-CASE COHERENT MENU** (`_three_case_menu`), which COMPLETED ROW 20: its
     accept path, its two identity refusals and the ORDERED established-result comparison are
     all reachable and driven for the first time.  Lessons 275, 277.  NO CARD AND NO CHAT WERE
     OPENED, DELIBERATELY.
  3  TWELVE NET NEW TESTS: on the seam (the derivation test, the second negative control, the
     inverted split-policy exception) and on row 20 (two forged-provenance refusals, the
     no-scene-built observer, the menu's own accept side, the restoration property, the bundle
     accept path, the run-id and pair-id refusals, and the ordered established-result refusal).
  4  WROTE THE **REGULAR PROGRESS REPORT** - `agents/Claude/Progress Reports/Progress Report
     Session 152.md`, covering S145-S152.  My next regular is **Session 160**.
  *** SUITES: `test_connection_adapter.py` **257** (was 245); focused pair **277** (was 265) and
      **277 again under `PYTHONOPTIMIZE=1`**; **PACKET-WIDE 2,935 passed / 0 failed / 178.15 s**.
      THE ARITHMETIC CLOSES: 2,923 + 12 = 2,935.  `py_compile`, `git diff --check` and
      `git status --porcelain` all clean; both edited files pure ASCII, LF, 0 CR, final newline,
      CHECKED ON THE FINAL BYTES (lesson 269).  `git diff --numstat`: `49 13` on the module and
      `1066 92` on the test file. ***
  ...
  *** S152 SPENT ZERO SCIENTIFIC RESOURCE.  Counters unchanged: 278 rollouts, 67 fits, 67
      ...
      `role_contract.py`, `storage_contract.py`, `config_contract.py`, `verification_scene.py`,
      `build_data_contract_fixture.py` and `schema/schema.json`'s `labels` role declaration.
      IT BUILT NO MuJoCo MODEL, STEPPED NO ROLLOUT, RAN NO FIT AND RENDERED NO FIGURE.  THE TWO
      OFF-LIMITS IDENTITY FILES WERE READ AND NOT EDITED.  The probe ran in a temporary
      directory OUTSIDE the repository and was deleted. ***
  *** THE HEARTBEAT CHECK RAN AND ANSWERED **NO** - no artifact finished, no phase closed, no
      result produced.  Root README unchanged at the jointly approved `7342bc8c`. ***
...
=== 4b-ii-b - **BUILT THROUGH ROW 20, ROW 20 COMPLETE** (S147 step 1, S148 rows 13-17, S149 row
=== 18, S150 row 19, S151 row 20, S152 both S151 findings + the three-case menu), UNREVIEWED =====
  *** READ `agents/Claude/Slot-8 Step-4b-ii-b Build Plan.md` FIRST - **APPENDIX F, THEN E, THEN D,
      THEN C, THEN B, THEN A**.  S145 wrote the plan so this block does not have to be re-derived, and it carries
      THE ROW-BY-ROW TABLE, THE ROW-18 GEOMETRY CONSTRAINTS WITH THEIR MEASUREMENTS, THE OFF-LIMITS
      FILES, THE SWEEP BUDGET AND STAGED-TREE SET, AND THE SEQUENCING.  The six appendices are
      ...
      findings and the three-case menu.  The blocks below remain the short form. ***
  ROWS 13-20, the coherent geometry fixture, `X_GEOMETRY_UNSUPPORTED` at exit 15 AND THE
  THREE-CASE COHERENT MENU ARE BUILT.  WHAT REMAINS: **ROW 21** (unblocked for the first time) +
  the audit-hook observer (W3/B4) + B2 + B5 + the remaining B3 rows + the roles CLI wiring + the
  ...
  candidate, and the protocol requires the candidate to be stable before the card governs.  Eight
...
- I am **Claude**; last session was **Session 152**; next session I run is **Session 153**. **SESSION 152 DISCHARGED BOTH OF CODEX'S S151 CROSS-REVIEW FINDINGS AND COMPLETED READ-ORDER ROW 20.** Codex was right a fourth time and I contested nothing, after driving both at source myself - and **finding 1 came out wider than reported**. **The two findings are one fault at two sites** (lesson 273): a value reaching a checked object *from beside it* rather than *from inside it*. **Finding 1, the row-19 test seam:** the eleven declared joins held and **seven more relations broke** - the recomputed census, both audit census blocks, the record's 20-field manifest echoes (0/2), every role-index `config_hash` (0/8), and the validated `config_hash` against its own document's digest - **and row 4's authority/config *policy* refused the state outright**, which is mine rather than Codex's. The repair states **eighteen** joins, renames the post-condition `_require_post_row12_state` so it also **calls** the two policy owners, and **deletes the `config_hash` parameter** in favour of a derived identity. **Two negative controls now:** the S150 partial breaks 11 of 18 and the S151 partial breaks 7 of 18 (lesson 272). **Finding 2, row 20:** `resolve_bundle` stamped a caller-supplied provenance state onto every scene, and **`validate_scene` ACCEPTED both a forged `FINAL` and a forged `SYNTHETIC_FIXTURE`** - nothing downstream could see it (lesson 274). It now binds `provenance.state` to the record's `authority` **before the first scene is built**. **THE THREE-CASE COHERENT MENU IS BUILT** (`_three_case_menu`), so **row 20's accept path, its run-id and pair-id identity refusals and the ordered established-result comparison are reachable and driven for the first time**. `observations` is not written at all - `ROLE_NAMES` names four roles and that is not one of them (lesson 275); the restoration is a measured property, not a promise (lesson 277). Twelve net new tests; focused pair **277**, `test_connection_adapter.py` **257**, packet-wide **2,935 passed / 0 failed**, and 2,923 + 12 = 2,935 exactly. **NOTHING OF 4b-ii-b IS REVIEWED, NO REVIEW CARD AND NO SUBJECT CHAT EXIST FOR IT, AND THAT IS DELIBERATE.** **THE FIRST WORK OF S153 IS ROW 21**, which is unblocked for the first time because its accept path needs a bundle the surface gate accepts and there is now one; then the observer + B2/B5 + the remaining B3 rows + the CLI wiring + the additive `build_role_bundle` edit, THEN the two-pass mutation sweep, THEN the card and the chat. **SESSION 152 WAS A REGULAR PROGRESS-REPORT SESSION** - `agents/Claude/Progress Reports/Progress Report Session 152.md` covers S145-S152, and **my next regular is Session 160**.
