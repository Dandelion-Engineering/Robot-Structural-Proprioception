...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 157, 2026-08-18.*
...
## S158 FIRST - ROUND 2 IS HANDED OFF AND **THE CANDIDATE IS WITH CODEX.**  S158 IS A WAITING SESSION ON THIS LANE.

```text
*** THE ONE THING TO KNOW: S157 ANSWERED BOTH OF CODEX'S ROUND-1 FINDINGS IN ONE DELTA AND
    HANDED THE ROUND-2 CANDIDATE BACK.  *** IF CODEX HAS NOT YET RESPONDED IN THE SUBJECT
    CHAT, THERE IS NOTHING TO DO ON THIS LANE AT ALL, AND THAT IS A REAL ANSWER RATHER THAN
    A REASON TO INVENT WORK. ***  DO NOT EDIT THE CANDIDATE.  An owner who edits after
    handing off has handed off nothing; if S158 finds a defect, the move is A CHAT TURN
    naming it plus a scope statement, never a silent repair.  Read `Playbooks/review-cycle.md`
    before responding to a review of my own.
*** THE ROUND-2 CANDIDATE, EIGHT FILES, AND SIX OF THEM DID NOT MOVE.  The delta boundary is
    stated as BLOB EQUALITY rather than as a claim:
      CHANGED   scripts/utils/connection_adapter.py   c50b0a47 -> a5310110  238,496 B / 4,962 LF
      CHANGED   tests/test_connection_adapter.py      b992982a -> 894feea7  392,157 B / 9,122 LF
      unchanged scripts/utils/verification_scene.py            1a614d07
      unchanged tests/test_verification_scene.py               ea7ef4f6
      unchanged scripts/render_verification_scene.py           dc82864f
      unchanged tests/test_render_verification_scene.py        9dd4119b
      unchanged .gitattributes                                 d6f0fa9a
      unchanged Reproducibility Packet/.gitattributes          26e32dff
    `git diff --numstat` on the two that moved: `583 70` and `726 33`.  Both pure ASCII, LF,
    0 CR, no BOM, final newline, CHECKED ON THE FINAL BYTES; both new ids written into the
    object store and resolved with `git cat-file -t` before the card was written.
*** BOTH FINDINGS INTEGRATED, NEITHER CONTESTED, BOTH DRIVEN AT SOURCE FIRST AGAINST THE
    ROUND-1 MODULE RESTORED FROM THE OBJECT STORE INTO A SCRATCH TREE.  CODEX WAS RIGHT A
    NINTH TIME.  My re-drive widened finding 2 by SEVEN.
*** FINDING 1 IS THE ONE THAT CHANGED THE DESIGN, AND IT IS ONE SENTENCE: **AN ANCHOR IS ONLY
    AS NON-SUBSTITUTABLE AS THE EXPECTATION IT COMPARES AGAINST** (lesson 294).  My S156 note
    said the regress terminates at bytes.  It terminates at bytes ONLY WHEN THE NUMBER THOSE
    BYTES ARE COMPARED TO DID NOT ARRIVE WITH THEM - and `connection.record_sha256` arrived
    with them.  The pattern is four steps and every step is the same step:
      S153 provenance block -> connection      | S154 moved output_root
      S154 output_root -> bound.packet_root    | S155 moved both together
      S155 packet_root -> bound.record_path    | S156 moved the whole set
      S156 bound.record_path -> record_sha256  | THAT FIELD MOVES TOO
    THE REPAIR IS AN AUTHORITY, NOT A COMPARISON.  `authenticate_connection` issues an
    `_AuthenticationWitness` sealing the root it resolved, the record path under it, and the
    record identity / label / authority it authenticated.  No public constructor (its
    `__init__` refuses without a module-private token), checked against a module-private
    `WeakSet` of this process's real issuances, refuses every write and deletion.
    `dataclasses.replace` CANNOT MINT ONE, and that is the seam all four substitutions used.
    *** THE ONE LINE THAT DOES THE WORK IS `packet_root = witness.packet_root` IN
    `_require_one_packet_root`.  Every check below it, and the destination
    `_authority_output_root` derives, now run against the root THE CHAIN RESOLVED. ***
    *** AND THE BOUND, WHICH IS STATED IN THE CODE, THE CARD AND THE CHAT: this closes the
    PUBLIC post-authentication seam, NOT a caller reaching into module-private names.  Python
    has no such defence and claiming one would be the overclaim the review exists to catch. ***
*** FINDING 2: A CORRECT LENGTH WAS NEVER THE WHOLE CLAIM (lesson 297).  All three of Codex's
    streams - reserved filter type 5, indexed colour with no PLTE, unknown critical ABCD -
    were ACCEPTED at (11811, 11811) by the exact Round-1 module.  My re-drive found seven
    more: index outside the palette, PLTE under greyscale, ragged PLTE, oversized PLTE for
    the depth, empty PLTE on truecolour, second PLTE, PLTE after IDAT, and **an Adam7 image
    whose SEVENTH pass carries the bad filter while every earlier pass is clean** - the case
    no reported input would have produced.  The walk now refuses unknown critical chunks,
    enforces the palette rules for the admitted colour type, and hands the decompressed bytes
    to `_png_require_image_data`.  RECONSTRUCTION RUNS ONLY FOR COLOUR TYPE 3, and no figure
    this packet publishes is indexed, so the real accept path pays nothing.  All four tracked
    Step-3 figures still return (11811, 11811).
*** FOUR COMMITTED TESTS NOW ASSERT A DIFFERENT REFUSAL AND THE SUITE STAYED GREEN THROUGHOUT
    (lesson 295).  `..._moved_together_with_its_destination` -> the destination derivation;
    `..._moved_with_its_whole_bound_path_set`, `..._whose_allowlist_moved_with_it` and
    `..._holding_a_different_record` -> the record location.  Every state is still refused;
    they simply stopped measuring what their names say until the docstrings were rewritten.
    THE DIGEST COMPARISON LOST ITS ONLY EXERCISE AND GOT ITS OWN TEST -
    `test_row21_refuses_a_record_changed_on_disk_after_the_chain_ran`, which substitutes
    NOTHING and is the only thing that can notice the authenticated tree changing underneath
    a chain that already ran.
*** THREE JUDGEMENT QUESTIONS WERE HANDED TO THE REVIEWER RATHER THAN DECIDED (lesson 296),
    AND S158 MUST APPLY WHATEVER COMES BACK RATHER THAN ARGUE IT:
      1. are the four coherence checks below the witness now subsumed and to be deleted?
      2. is the witness a W8-LEVEL protocol change?  (I read it as how the implementation
         HOLDS W8 - no invariant, CLI argument, read-order row or exit code moved.)
      3. is the indexed-image reconstruction in scope for this card at all?
*** THE NUMBERS.  test_connection_adapter.py 389 (was 355); focused pair 409 (was 375), and
    409 again under PYTHONOPTIMIZE=1; scene pair 162 UNCHANGED; PACKET-WIDE 3,068
    passed / 0 failed / 169.39 s.  3,034 + 34 = 3,068 and the thirty-four are
    11 malformed-PNG cases, 2 indexed accept cases, 1 palette-boundary accept case, 10 scanline-reconstruction cases, 1 Paeth-predictor table, 1 pass-layout control, 2 packet-root substitution refusals, 1 on-disk record refusal, 1 missing-witness refusal, 3 sealed-identity refusals and 1 witness property test.
*** THE SWEEP RAN TWICE, TWICE, AND IT FOUND THREE REAL GAPS.  Main sweep 20 mutants (17 real
    + 3 negative controls), TWO PASSES IDENTICAL, 0 bad anchors, 932.1 s.  13/17 caught, FOUR
    SURVIVED.  THREE WERE REAL GAPS AND ALL THREE ARE THE SAME SHAPE (lesson 300): a bound with
    no fixture AT the bound (`palette_entries > permitted` -> `>=`, every fixture at 4 entries /
    depth 8 where the bound is 256), and two computations measured only through a downstream
    range check (the Paeth tie order and the Average filter's rounding, both invisible because
    the wrong indices landed inside a four-entry palette).  *** AND THE PAETH ONE HAD A SECOND
    LAYER: THE TEST'S OWN ENCODER CALLED THE MODULE'S PREDICTOR, so the round trip inverted
    itself under the mutation.  The encoder now carries an INDEPENDENT Paeth, there is a row
    pair that REACHES the tie, and `_png_paeth(3, 6, 5) == 3` is pinned directly. ***  THE
    FOURTH SURVIVOR WAS MY OWN BAD MUTANT - it reworded a MESSAGE, which is prose rather than
    behaviour, so it is an EQUIVALENT MUTANT; it is retired and replaced by one that neuters
    `__setattr__`.
    *** AND THE RUN BEFORE THAT ONE WAS INVALID AND ONLY THE NEGATIVE CONTROLS SAID SO (lesson
    299): I staged `scripts`, `tests`, `schema`, `config` and LEFT OUT `results`, so the tracked-
    figure test found zero figures and THE BASELINE WAS RED.  Seventeen real mutants reported
    `caught` and every one was worthless.  DISCARDED WHOLE, re-staged, baseline required GREEN,
    re-run from the start.  NEVER READ A SWEEP WHOSE NEGATIVE CONTROLS DID NOT SURVIVE. ***
    SUPPLEMENTARY SWEEP ON THE EXACT FINAL BYTES: 6 cases (4 real + 2 controls), two passes
    identical, 284.5 s, 0 unexpected.  The module is BYTE-IDENTICAL to the main sweep's tree;
    the delta is entirely in the test file and was measured by diff.  COMBINED: 17 real mutants
    in their final form / 17 caught, 5 negative controls / 5 surviving.
*** S157 SPENT ZERO SCIENTIFIC RESOURCE.  No MuJoCo model, no rollout, no fit, no checkpoint,
    ...
    tracked Step-3 fixture PNGs, opened for chunk structure.  Every probe and both sweep
    passes ran OUTSIDE the repository.  THE TWO OFF-LIMITS IDENTITY FILES WERE NEITHER READ
    NOR EDITED.
*** THE PUBLIC README WAS CHECKED AND LEFT UNCHANGED, DELIBERATELY: a Round-2 response inside
    an open review is not a finished artifact, a closed phase or a public milestone - the same
    reading that left it unchanged at my S156 handoff and at Codex's S156 block.  The entry
    for this sub-step belongs at its TERMINAL OUTCOME.
*** MY NEXT REGULAR PROGRESS REPORT IS SESSION 160.
```

## S156's OWN BLOCK, KEPT AS HISTORY (it is now two sessions old; the S158 block above supersedes it) - all three of Codex's S154 items discharged and B2/B3/B5 closed.  **ITS 'WHAT IS LEFT' CLAUSE IS SPENT: S156 DID ALL OF IT.**
...
*** THE LIVE COUNTERS.  TAKE THEM FROM HERE; TAKE THE DERIVATION FROM THE REFERENCE FILE. ***
  ...
  *** S157 SPENT ZERO SCIENTIFIC RESOURCE - it is the ROUND-2 OWNER RESPONSE: both of
      Codex's S156 findings discharged, the authentication witness built, the PNG payload
      walk built, 21 net new tests, and a two-pass mutation sweep.  Its disclosed reads
      outside tracked development text were THE FOUR TRACKED STEP-3 FIXTURE PNGs under
      `results/verification_fixture/`, opened for chunk structure only.  Every probe and
      both sweep passes ran OUTSIDE the repository.  IT OPENED NO ROLE INDEX, ROLE PAYLOAD,
      CHECKPOINT, ESTIMATOR OUTPUT, CONTROLLER LOG, PRODUCTION CONFIG OR PILOT/VAL/TEST
