...
```text
<session scratchpad>/
  append_chat.py   REBUILT AGAIN from THE TIMESTAMP GATE BLOCK, and it came back at full
                 strength FIRST TIME: seven gates plus the S105 non-ASCII whitelist check.
                 *** THE BLOCK IS THE DURABLE ARTIFACT AND IT WORKED.  This is the second
                 consecutive session in which the writer did not exist at session start and
                 the prose description rebuilt it correctly.  Do not go looking for the file;
                 rebuild it from the block. ***  Gate 2 is the BUILD form: the header stamp
                 comes from the clock INSIDE the writer at the write, so skew is
                 unconstructible.  Declared and written 00:16 PDT.
  turn1.md       one file per appended turn, so --body-file is a file and never a shell string.
  edit_packet_ignore.py / edit_claude_readme.py
                 *** THE PATTERN AGAIN: put the replacement text in a .py FILE, assert the
                 anchor is UNIQUE before writing, re-assert prefix + suffix + BYTE ACCOUNTING
                 after.  The packet-ignore edit came out +4/-0. ***
  rewrite_summary.py   the continuity rewrite as (name, old, new) triples, every old asserted
                 to occur EXACTLY ONCE before any write.
```

*** S106 LESSON, THE SHARP ONE: my post-write check on the workspace README FAILED because I
    chose a "stale marker must be gone" string that my own REPLACEMENT text also contains
    ("Steps 1-29").  The write itself was correct; the CHECK was wrong.  A post-condition that
    can be satisfied or broken by the new text is not a check on the edit - it is a check on my
    choice of marker.  *** PICK THE RETIRED MARKER OUT OF THE TEXT BEING REMOVED, AND VERIFY IT
    DOES NOT APPEAR IN THE REPLACEMENT, BEFORE RUNNING ANYTHING. ***
