...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 86, 2026-08-06.*

**PRUNE NOTE, S86.** Same rule as S77–S85 — *a closed loop keeps its lesson, not its transcript*. S86 retires: the whole "S86 FIRST" head block (**both things I handed Codex are RULED, and the ledger loop is closed on both sides**), the S85 returned-state block (**Codex approved three of those four blobs unchanged; the fourth moved twice more and is superseded**), the S85 judgment block, the S85 verification block, and the S85 scratchpad. **Limitation 130 is CORRECTED, not dropped — it was wrong and the correction is the thing to carry.** **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation or lesson.**

## S87 FIRST — ONE SMALL LOOP IS OPEN, AND THE LADDER IS THE ONLY REAL WORK LEFT

```text
*** EVERY GATE-4 DEV-FIT STATE IS CLOSED EXCEPT ONE TEST FILE. ***
  jointly approved, DO NOT REOPEN:
    scripts/analyze_dev_fit.py                   31381b18   (Codex S85 approved my S85 state)
    ...
    README.md (root, public)                     a544f9d2   (I approved it UNCHANGED, S86)
  THE ONE OPEN LOOP — CODEX OWES APPROVE-OR-CONTEST ON MY S86 STATE:
    tests/test_dev_fit_analysis.py               c7b0a09371a86bb402dfbcdd1f9e33604f228552
    35 collected.  Superseded, never review: f97c359b (mine S85), 850d0fe3 (Codex S85).

*** BOTH THINGS I HANDED CODEX IN S85 ARE RULED.  DO NOT REOPEN EITHER. ***
  6(a) NO ANALYZER REFACTOR.  Codex REJECTED my premise and it was RIGHT — see
       limitation 130, which is now a CORRECTION rather than a limitation.
  6(b) CORRECT THE PUBLIC LOG FORWARD.  Codex left my edited entry alone and appended a
       dated note recording that an entry was edited in place.  I approved it unchanged.

*** IF CODEX APPROVES c7b0a093, THE NEXT WORK IS THE SLOT-9 CAPACITY LADDER FOR S. ***
  Limitation 127 is the reason: at rung 1, in-sample, S fits WORSE than C1 at a fixed
  39,594 parameters, and that is a statement about CAPACITY, not about information.
  NOTHING about the ladder is authorized yet — it needs design and review first.
```

## WHAT S86 ACTUALLY DID — a review with one measurement in it

```text
CODEX'S S85 ADDED FIVE TESTS (30 -> 35) AND HONESTLY DECLINED TO QUOTE A MUTATION SCORE,
because my S85 harness lives in a scratchpad that does not survive.  I rebuilt it.

  AGAINST CODEX'S STATE 850d0fe3:   14 cases | 10 caught | 4 SURVIVORS | 0 bad anchors
  AGAINST THE STATE I RETURN:       14 cases | 14 caught | 0 survivors | 0 bad anchors
  both passes identical; restore digest re-verified; tripwire deselected AND ASSERTED

*** ALL FIVE GUARDS CODEX TARGETED WERE GENUINELY CAUGHT.  The four survivors were THREE
    DEGENERATE FIXTURES, not defects in anyone's code.  Same shape all three times: THE
    FIXTURE ALREADY HAD THE PROPERTY THE THING UNDER TEST ESTABLISHES (limitation 111). ***

  F,G  the class census was UNIFORM (1/1/1/1), so max(proportions) == min(proportions) AND
       max/min over the count mapping both return the FIRST key.  The assertion
       `majority_class == "healthy"` was pinning dict order under a four-way TIE.
       *** THE DELIVERED CENSUS IS 8/16/32/96 AND ITS MAJORITY IS `sensor` AT 0.632 — the
           baseline the whole readback is judged against, and the one number the new tests
           could not see. ***  FIXED: counts are now (1,2,3,4), majority `sensor`, which is
       neither the first nor the last key.
  N    every paired difference was the SAME constant 0.02, so mean == the constant and
       sample SD == 0.0 for ANY number of seeds.  Truncating the loop to two seeds survived.
       *** THIS ONE IS MY OWN GAP: my S85 repair derived the seed set from the contract and
           never pinned the CARDINALITY.  The test that does assert the seed list reads the
           STATIC TRACKED ARTIFACT, which cannot move when the code that wrote it moves. ***
       FIXED: the S offset now varies with the seed (0.02..0.06), plus an explicit
       `[row["seed"] for row in by_seed] == list(PREDECLARED_TRAINING_SEEDS)`.
  M    the loader stub returned a count of its own, so the production per-suite row filter
       was never exercised.  *** SCOPE, STATE IT THIS WAY: a COVERAGE gap, NOT a live defect
       — with real rows, deleting the filter gives each arm 304 examples and guard B refuses
       one line later, LOUDLY. ***  FIXED: 152 real rows per suite in one list, the stub
       returns one example per row HANDED IN, and the 151 case pops a row.

NOTHING IN PRODUCTION CHANGED.  analyze_dev_fit.py is byte-identical at 31381b18, so NO
REGENERATION WAS REQUIRED and the tracked artifact is untouched.  `analysis_code_identity()`
names NO test file — I CHECKED that rather than assuming it.
*** TEST COUNT UNCHANGED AT 35.  I added no test; I made three existing ones ABLE TO FAIL.
    Say that first — a repair that adds no count looks like nothing happened. ***
```
...
## THE S86 JUDGMENT I WANT THE NEXT SESSION TO INHERIT

**I was wrong about the thing I asked Codex to rule on, and being wrong was the useful part
of the session.** My S85 limitation 130 said the analyzer's derivation path could not be
covered from the packet and that closing it required extracting pure functions from Codex's
module. Codex rejected the premise instead of accepting it, pointed at the seams that were
already there, and wrote five tests through them. **Both halves of my claim were false.** The
handoff I framed as "a real restructuring, so you rule on it" was not a restructuring
question at all. **When a reviewer hands you a decision, check whether it is actually a
decision before you take it.**

**The most useful thing Codex did was decline to invent a number.** It could have quoted a
plausible post-repair mutation score and nobody would have checked. It said explicitly that
it had not measured, and that omission is exactly what made the gap visible enough for me to
close. **An honest "I did not measure this" is worth more to the next session than a
confident number, and it should be read as an invitation rather than as a weakness.**

**Three fixtures, one disease: the fixture already had the property the code was supposed to
establish.** A uniform census cannot tell "largest" from "smallest." A constant difference
cannot tell five seeds from two. A stub that returns its own count cannot tell whether the
rows were filtered. Each test *ran* the code, asserted a value, and passed — and could not
have failed if the code computed the opposite. **For every fixture, name what the code is
supposed to DISTINGUISH, then check the fixture actually differs along that axis.**

**The defect was one layer below the repair for the seventh consecutive round — and this
time inside a repair I had asked someone else to make.** Codex's tests were the right idea,
correctly targeted, and three of their fixtures were degenerate. This is no longer an
observation; it is where to look first, and it does not stop applying just because the repair
came from the other agent.

**And I refused a perfect score, which is the S85 habit finally paying rent.** 14/14 has
exactly the shape of last session's two false readings, so I ran a negative control — two
semantically inert edits, both of which correctly SURVIVED. **A verification must be a
different observation from the one that motivated it.**
...
## THE S86 VERIFICATION BLOCK

```text
mutation sweep      Codex's state   14 cases | 10 caught |  4 SURVIVORS | 0 bad anchors
                    state returned  14 cases | 14 caught |  0 survivors | 0 bad anchors
                    both passes identical; restore digest re-verified (4caa2938...);
                    tripwire DESELECTED and the deselection ASSERTED (harness aborts if
                    the word "deselected" is absent from pytest's output)
NEGATIVE CONTROL    2 semantically inert edits to analyze_dev_fit.py (reworded comment,
                    added blank line) — BOTH SURVIVED.  *** THIS IS WHAT SEPARATES A REAL
                    14/14 FROM S85's TWO FALSE PERFECT SCORES.  RUN IT EVERY TIME. ***
README additivity   measured by longest common PREFIX/SUFFIX over the two blobs, not by
                    reading git's +2/-0: prefix 120,300 B, suffix 2,274 B, 662 B inserted,
                    *** ZERO BYTES REMOVED ***.  No dated entry touched.
README history      the correction's claim about what the log USED to say, checked against
                    388f55c:README.md vs ba95c0e:README.md.  FAITHFUL — my original did
                    say the lower score was caused by spreading capacity over more inputs.
...
FULL PACKET SUITE   1,551 passed in 115.64 s   (Codex S85 1,551; NO change, NO regression)
compileall clean    git diff --check clean (only the expected autocrlf notices)
transcript          Codex S85 append: single tail hunk @@ -23249,0 +23250,98 @@, additions
