...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 102, 2026-08-09.*

**PRUNE NOTE, S102.** Same rule as S77-S101 - *a closed loop keeps its lesson, not its transcript*. S102 retires: the "S102 FIRST" head block (superseded by the S103 one below), and the section-12 five-step ladder (ALL FIVE WERE ALREADY CLOSED and nothing reopened them - the sweep is finished as a measurement and the open work is now C7's own three gates, which are stated in the new head block). **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation or lesson.** The S101 audit set, the two roots, the Step-4 shape, finding AU, finding AT, the declined guards and every scope statement are all KEPT - the Step-4 shape in particular is now live again, because a C7 execution will need exactly that act.

**SUPERSEDED PRUNE NOTE, S101.** Same rule as S77-S100 - *a closed loop keeps its lesson, not its transcript*. S101 retires: the "S101 FIRST" head block (superseded), **the whole four-gate ladder (ALL FOUR ARE NOW CLOSED)**, the plan-history location block (the plan is consumed; its one lesson is kept in a line), the S100 136-check audit block (superseded in coverage by the S101 176-check set), the S98 62-check block (superseded twice), the S99 mutation-sweep table (the repair it audited has now completed a full 42-fit run, which is a stronger fact than the table), and the Step-4 authorization block's operational detail (both halves are spent and no further Step-4 act is in view - the four residuals and the concurrent-writer lesson are kept). **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation or lesson.**

## S103 FIRST - C7 IS BUILT AND ITS REVIEW LOOP IS OPEN ON CODEX. DO NOT RUN IT.

```text
*** DO NOT COMPUTE THE SECTION-5 DESCRIPTIVE READ.  Not headroom, not pair_constraint, not
    any curve shape, not the crossing points, not paired_range, not the derived label, not
    m(c) or s(c).  Section 5's whole purpose is that the interpretation is fixed before
    anyone sees the curve, and a probe that "just checks the numbers are sane" spends the
    pre-registration and cannot give it back.  I did not compute it in S102 and neither
    should S103 until all three gates below have been passed IN ORDER. ***

THE THREE REMAINING GATES.  NONE IS INFERRED FROM THE ONE BEFORE IT.
  1  the C7 CODE loop closes on ONE state both agents explicitly approve   <- OPEN, on Codex
  2  ONE C7 execution under its OWN exact command/input/output authorization
  3  section 5.4 applied JOINTLY to the exact reviewed output

WHERE GATE 1 STANDS.  Codex built C7 in its S101 and approved two exact states; I reviewed
them in my S102, found TWO defects, repaired both, added three tests, and returned an edited
state I explicitly approve.  CODEX OWES THE OWNER RE-REVIEW.
  Reproducibility Packet/scripts/analyze_capacity_sweep.py
    blob b9043fa266dc7c35a6acdb240216ae0ec3337f6e
    canonical == raw 7eca4016d7ffb73c15ec1e35642e5f6e1ecb95a7c6757e72cc875cf79f87ffbe
    44,600 B / 1,088 lines / LF / pure ASCII / no BOM / final newline
  Reproducibility Packet/tests/test_capacity_sweep_analysis.py
    blob a81d35c952fba158f647a64b9cd13bad0c301c93
    canonical == raw bd8c36316b4be433cac0000ef2597137cb35b68b0f5407c7b992764d9976d229
    29,957 B / 805 lines / LF / pure ASCII / no BOM / final newline / 24 tests
  SUPERSEDED, never review or build from: 5dcc0947 / c33e21f5...fa35fe27 and
    5e4497fd / 1d95cdc9...05842586 (both Codex S101).
  Suites at the returned state: C7 24, capacity+C7 241 (also under python -O), packet 1,792.

*** IF CODEX APPROVES THOSE BYTES, GATE 1 CLOSES AND GATE 2 IS A STEP-4-SHAPED ACT.  See
    "THE STEP-4 SHAPE" below - it is live again.  A C7 run writes ONE artifact
    (capacity_sweep_analysis.json) through an EXCLUSIVE create, so the destination is
    single-use: a second run needs a different --output-dir, which is a second act. ***
```

## FINDINGS AV AND AW - MY S102 C7 REVIEW. Do not undo either repair.

```text
AV  THE TWO ARM KINDS ARE PERSISTED IN DIFFERENT NUMERIC DOMAINS, AND evaluate_arm_context
    COMPARED BOTH EXACTLY.  It recomputed every arm's classification scores and required
    metrics == record.  Right for the forty COMPLETED arms; UNSATISFIABLE for the ten
    REUSED anchors, because the two kinds have two writers:
      COMPLETED  curve_arm_document copies score_arm's mapping through UNROUNDED
      REUSED     the anchors come from dev_fit_analysis.json, whose WHOLE report goes
                 through analyze_dev_fit.rounded() -> round(x, 12)
    MEASURED FROM THE PUBLISHED ARTIFACTS ALONE, no data read, no model loaded: every
    per-class F1 is a 2TP/(2TP+FP+FN) rational over 152 examples, so the denominator is an
    integer <= 304.  Reconstructing the unique such rational behind each published 12-dp
    value:  32 of 40 per-class values and 10 of 10 anchor macro-F1 values DIFFER from
    their persisted rendering, while 40 of 40 new arms render with MORE than 12 decimals.
    Arms evaluate in (channels, suite, seed) order, so a real run would have re-scored the
    twenty arms at 16 and 24 and then refused at the FIRST anchor.
    *** DRIVEN, NOT ONLY ARGUED: I loaded the 304 authorized dev rows and one approved .pt,
        built the arm through Codex's own validate_arm, and watched Codex's own function
        refuse - while rounded(raw) == stored held for macro_f1, accuracy AND per-class. ***

    THE REPAIR - require_recomputed_scores_match(arm, metrics).  TWO PROPERTIES:
      1  COMPLETED keeps EXACT equality.  The raw float is what was written, so the
         strongest check is available there and must not be weakened.
      2  REUSED is compared at the approved analyzer's own boundary, using THAT FILE'S
         rounded(), IMPORTED and never restated.
      AND the stored anchor value is required to be at that boundary TOO - both directions,
      so a future change that stopped rounding fails loudly instead of quietly becoming a
      comparison of a value with itself.
    *** DO NOT "SIMPLIFY" THIS BY ROUNDING BOTH SIDES.  That fixes ten arms by giving up a
        real check on forty.  Mutation M2 exists to catch exactly that edit. ***

AW  A SECOND NETWORK CONSTRUCTION SITE, IN THE ONE FILE C5's TEST CANNOT SEE.  C7 built
    TemporalAttributionNet(..., enforce_rung1_band=True) itself.  The AST test
    test_the_rung1_band_guard_cannot_be_turned_off_from_the_command_line parses
    capacity_sweep.py AND NOTHING ELSE, so a second site in another file is outside its
    reach and the sweep suite stays green while two definitions drift.  Repaired to
    sweep.build_network(...), moved ABOVE the try so a channels/seed refusal propagates as
    itself rather than being relabelled "a capacity checkpoint cannot be loaded".
    TemporalAttributionNet is no longer imported or referenced in the reader.
...
WHY THE SUITE MISSED BOTH - THE PART TO CARRY.  Both sat under 21 passing tests, and both
  ...
  TWELVE DECIMALS - and it is a COMPLETED arm, so the anchor branch is never entered.  The
  test binding reused anchors builds its approved rows FROM its record rows, so both sides
  come from one source.  My three added tests make non-degeneracy an EXPLICIT ASSERTION
  (rounded(fixture) != fixture) rather than a property of the numbers I happened to pick.

THE FOUR-CASE TWO-STATE SWEEP, each case run twice with identical results, every restore
  digest-verified in a finally:
    M1 restore the exact comparison for both kinds (the handed-over behaviour)   CAUGHT
  ...
  Each caught by the test that NAMES it, not merely by the suite.  M1 is the negative
  control that matters: it reproduces what was handed to me and the new test goes red.

THE S102 SUFFICIENCY CHECK - 13/13, and it is the shape to reuse before gate 2.  Drive the
  reader's WHOLE authentication chain against the real state and STOP BEFORE derive_analysis:
  the three inputs parse and hash to the approved digests; validate_envelope accepts and
  sources BAR 0.05 and s(32) 0.149635726834; validate_arms returns fifty normalized arms
  (10 REUSED / 40 COMPLETED); load_development_context returns 152+152 rows; all fifty
  checkpoints resolve inside their namespace and 50 of 50 digests match; and the repaired
  comparison accepts 10 of 10 anchors where it refused 10 of 10 before.
  *** THE ANCHOR-SD HARD REFUSAL IS SOUND AND I KEPT IT: recomputing s(32) from the record's
      own anchor values and rounding to 12 dp reproduces the published 0.149635726834
      exactly (0.1496357268341403, 3.6e-13 of margin to the nearest rounding boundary). ***

TWO THINGS I MEASURED AND RECORDED RATHER THAN RAISED - do not "fix" either in a later
  ...
  REASONING and not only the observation.

protocol_p.canonical_text_sha256 computes all three of C7's input digests and is in NEITHER
  identity set.  That is the RECORDED protocol-P scope statement, NOT a new finding, and I
  did not reopen it.
```
...
## THE PHASE-3 ITEM I RECORDED AND DID NOT FIX

```text
The packet's own README does not mention the capacity sweep AT ALL - zero occurrences of
`capacity_sweep`, measured S101 - and the 55 git-ignored checkpoints have no documented
clean-machine recovery path.  Codex named the second half in its S100; the first half is
mine.  BOTH are Phase-3 assembly obligations for a packet that must run end to end from a
copy of its own folder.  NOT a blocker on anything currently open.  Do not fix it inside a
review session: a packet edit mixed into a review is how a review stops being one.
```

## THE TIMESTAMP GATE - built S100 after my own failure, and it held in S101
...
- **C7, THE READ-ONLY CAPACITY-SWEEP ANALYSIS - CODEX OWNS IT, I AM THE REVIEWER, AND THE LOOP IS OPEN ON CODEX.** `Reproducibility Packet/scripts/analyze_capacity_sweep.py` + `tests/test_capacity_sweep_analysis.py`. Codex built and approved `5dcc0947`/`5e4497fd` in its S101; **my S102 found AV and AW, repaired both, added three tests, and returned `b9043fa2`/`a81d35c9` with my explicit approval.** See the AV/AW block and the "WHAT C7 IS" block at the head of this file. **DO NOT RUN IT** - execution is gate 2 and needs its own authorization.
...
- **C7, THE READ-ONLY ANALYSIS - REVIEW LOOP OPEN ON CODEX, DO NOT RUN IT:** `Reproducibility Packet/scripts/analyze_capacity_sweep.py` blob `b9043fa266dc7c35a6acdb240216ae0ec3337f6e` / canonical == raw `7eca4016d7ffb73c15ec1e35642e5f6e1ecb95a7c6757e72cc875cf79f87ffbe`, 44,600 B / 1,088 lines; and `Reproducibility Packet/tests/test_capacity_sweep_analysis.py` blob `a81d35c952fba158f647a64b9cd13bad0c301c93` / canonical == raw `bd8c36316b4be433cac0000ef2597137cb35b68b0f5407c7b992764d9976d229`, 29,957 B / 805 lines, **24 tests**. Both LF, pure ASCII, no BOM, final newline. Codex built and approved `5dcc0947`/`5e4497fd` (21 tests) in its S101; I found **AV** and **AW**, repaired both, added three tests and returned the state above with my explicit approval in my S102. **Superseded, never review or build from: `5dcc0947`/`5e4497fd`.** **THE NEXT THING TO TOUCH THIS PAIR IS CODEX'S OWNER RE-REVIEW.** Read the AV/AW block and the "WHAT C7 IS" block at the head of this file before anything else; this bullet is an index. **EXECUTION IS A SEPARATE GATE and its output destination is single-use (exclusive create).**
...
## Scratchpad (S101, superseded - still the right shapes)

```text
<session scratchpad>/
  audit_a_result.py  *** THE EXACT-STATE AUDIT, 141 checks, IMPORTS NOTHING from the
                     producer.  Successor to S100's audit_a_plan.py and the right starting
                     point for reviewing ANY published artifact.  Keep canon()/rawd()/blob()
                     side by side, dig() for retrieving a value through the field path the
                     DOCUMENT names, walk() for driving a forbidden-content rule as a SEARCH
                     over every member name and string leaf, and the two design-text parsers
                     that rebuild the expected arm set from the frozen document's own prose. ***
  audit_b_gates.py   Parts B and C in one file: the module's read-side gates driven with
                     negative controls, then the TEMP-REPLICA namespace probe.  It
                     sys.path.inserts the packet's scripts/ dir so `utils.capacity_sweep`
                     imports; argparse'd on --project-root.
  append_chat.py     SEVEN gates: prior digest, no CRLF, header uniqueness, byte append,
                     prefix re-hash, physical-tail ASSERTED, and header timestamp vs the
                     clock AT THE WRITE (+/-120 s).  Stamp the header in the SAME command.
  readme_append2.py  byte-level Live-Run README appender for the CURRENT mixed-EOL tail
                     (`result.\n\r\n---\r\n\r\n## Follow along`); anchor asserted unique,
                     prefix AND suffix asserted after the write.
  rewrite_summary.py / fix_pointer.py / readme_workspace_update.py
                     *** THE PATTERN THAT MATTERS: put every replacement string in a .py
                     FILE with a uniqueness assertion before the write and a re-read after.
                     Backticks in a `python -c` string get expanded by bash - that cost me a
                     README write this session. ***
  turn1_review.md    the review turn.  Pure ASCII before appending; the transcript's older
                     bulk is CRLF and my appends are LF.
```

## Scratchpad (S100, superseded - still on disk and still the right shapes)
