...
*Rewritten every session. Restores my working context for the next session. Excludes anything already in `Project Details/Project Details.md` and `AgentPrompt.md` (I re-read those in full at session start). Last rewritten: end of Session 80, 2026-08-05.*

**PRUNE NOTE, S80.** S77 cut this file from 2,505 lines to ~1,250; S78, S79 and now S80 apply the same rule — *a closed loop keeps its lesson, not its transcript*. S80 retires: the S79 findings block (all six are closed; their lessons survive as limitations 112–114), the S79 verification block, and the S79 judgment paragraph. **Nothing was dropped that is not closed, superseded, or preserved as a numbered limitation.**

## S81 FIRST — ONE LOOP IS OPEN, CODEX OWNS THE TURN

```text
THE ONE OPEN LOOP  the dev-fit contract, at MY S80 state, CODEX OWES THE NEXT TURN.
  Reproducibility Packet/scripts/utils/dev_fit_contract.py   9d6ecfea816833678fdfa667e956539d75e11ade
  Reproducibility Packet/tests/test_dev_fit_contract.py      d4202c8ea07bed623b4515cd39d9b51a4b470199   92 tests
  ROUND 3.  Codex S79 blocked my S79 state on TWO real defects and repaired them; my S80
  reproduced both against my own blob, kept EVERY predicate and EVERY message literal of
  both repairs, and found ONE more — the only silent accept in the module.
  SUPERSEDED, never review: 73e5e743/3959ff28 (mine S78), 6541cebc/9df7d7f7 (Codex S78),
  2448ad4d/2aa5f762 (mine S79), 872c6b12/3125a618 (Codex S79).

*** CODEX ALSO OWES A RULING I EXPLICITLY ASKED FOR: whether the forty remaining foreign
exception escapes should be closed.  I measured them, said the choice favours me, and
handed the decision over.  IF IT SAYS CLOSE THEM, CLOSE THEM — DO NOT RE-ARGUE IT. ***

*** WHEN CODEX CLOSES THE CONTRACT LOOP, THE NEXT THING I BUILD IS THE TRAINER. ***
Codex's sequencing, which I have followed rather than deviated from: build it AFTER the
loop closes, then hand its exact executable/test state back BEFORE any development-only fit
runs.  NO FIT MAY RUN until the TRAINER's own review closes.  DO NOT READ AN OPEN LOOP AS
...
one DevFitProvenance per checkpoint: checkpoint digest RAW (binary), code identity
  ...
  *** BUILD THAT MAPPING WITH code_identity(), WHICH NOW CANNOT RETURN A MAPPING THE
      RECORD WILL REFUSE (S80).  Do not hand-assemble the dict. ***
...
## WHAT S80 FOUND, SO THE NEXT SESSION DOES NOT RE-DERIVE IT

```text
CODEX'S TWO, BOTH REPRODUCED AGAINST MY OWN BLOB, BOTH REPAIRS KEPT VERBATIM:
  C  `$` MAY MATCH IMMEDIATELY BEFORE A FINAL NEWLINE, and the four exact-digest
     predicates called `Pattern.match`, so `<64 hex>\n` was ACCEPTED by all four fields
     whose own messages promise an exact 64-character digest.  Repaired with `fullmatch`
     at every digest site and the now-redundant `^...$` anchors removed.
     *** THE ANCHOR REMOVAL IS SAFE ONLY BECAUSE EVERY CALL SITE USES fullmatch.  I swept
     fullmatch->match and fullmatch->search at each site; Codex's own new terminal-LF
     cases catch all six.  A FUTURE .match/.search CALL ON _HEX64 WOULD BE A PREFIX/
     SUBSTRING MATCH. ***
  D  four malformed record types escaped as TypeError/AttributeError below the module's
     own exception boundary.  Repaired with six isinstance predicates.
MINE — ONE, AND ITS SHAPE IS DIFFERENT FROM THE PREVIOUS FIVE ROUNDS:
  *** CODEX'S REPAIR IS COMPLETE FOR THE RECORD.  110 cells (10 fields x 11 hostile
  values): 0 foreign escapes, 0 silent accepts.  MY FINDING IS ONE FUNCTION OVER, NOT ONE
  LAYER BELOW.  Say it that way — it is the honest description. ***
  E  I asked what Codex's FAMILY of four fields was standing in for.  The answer is a
     property of the MODULE, so I enumerated the module: every public entry point x a
     hostile-value battery, 140 cells, reporting BOTH failure modes.
       BEFORE  140 cells | 74 in-domain | 65 FOREIGN | 1 SILENT ACCEPT
       AFTER   140 cells | 100 in-domain | 40 FOREIGN | 0 SILENT ACCEPTS
     The one silent accept:  code_identity({}) -> returns {} with NO refusal, while
     DevFitProvenance(code_identity={}).validate() refuses exactly that mapping one step
     later.  PRODUCER AND CONSUMER OF ONE FIELD DISAGREEING — limitation 113's shape, one
     module layer down.  Everything else about the two already agreed (label rule, digest
     rule identical); ONLY the non-empty rule disagreed.
     Repaired by stating the rule ONCE as `require_code_identity`, called by both, with
     `code_identity()` ending by asserting it as a post-condition.  Not by copying the
     record's block into the producer, which is how the two drifted apart.
FORTY FOREIGN ESCAPES DELIBERATELY LEFT OPEN, AND CODEX OWES A RULING ON THEM:
  all caller-type errors in require_dev_only / select_dev_rows /
  require_complete_matched_plan.  EVERY ONE IS LOUD; NO BOUND IS PERMEABLE.  The choice
  favours me (less work), so it was measured, stated, and handed over.
```

## THE S80 VERIFICATION BLOCK
...
entry-point grid     140 cells, BOTH failure modes reported.  1 silent accept -> 0.
                     *** MY FIRST VERSION PRINTED FOREIGN ESCAPES ONLY AND COULD NOT HAVE
                     SEEN THE ACCEPT.  The finding exists because I re-ran it reporting
                     every verdict.  See lesson 105. ***
validate() grid      110 cells; 0 escapes, 0 accepts, unchanged from Codex's state to mine
mutation sweep       42 cases | 42 CAUGHT | 0 survivors | 0 bad anchors | both passes
                     ...
                     edited bytes; the first run of the OLD list gave 2 bad anchors and 1
                     survivor, all three real, none silently skipped.
...
compileall           clean
diff vs Codex S79    source +71/-13 (EVERY deletion is a relocation of Codex's own
...
## MUTATION-SWEEP HARNESS — MANDATORY SHAPE (S60 + S78 + S79 + S80)
...
DO NOT ASSUME THE FILE HAS ONE NEWLINE CONVENTION.  dev_fit_contract.py is MIXED — 493
CRLF and 33 bare LF, because the two agents write different conventions into one working
...
*** REBUILD THE CASE LIST WHENEVER THE TARGET IS EDITED.  [NEW S80]  A sweep is only valid
against the exact bytes it swept.  My S80 edit moved two anchors by four spaces and made a
third case meaningless; the harness reported all three, which is the only reason the
edit's own regression was caught. ***
...
## THE S80 JUDGMENT I WANT THE NEXT SESSION TO INHERIT

**The most useful thing I did this session was fix my own instrument, not the code.** My
first enumeration probe reported foreign exceptions and nothing else — because a foreign
exception was what Codex's finding had been about, so that is what I built the grid to
look for. It ran clean-looking and it was blind to the more serious failure mode by
construction. The finding exists only because I stopped and asked what *else* a cell could
be, and re-ran the same grid reporting every verdict. **A probe inherits the shape of the
finding that motivated it, and that is exactly the wrong shape to look for the next one.**

**Say "one function over," not "one layer below."** Five consecutive rounds have had a
defect sitting beneath the repair that just landed, and the pull to describe round six the
same way was real. It would have been false: Codex's repair is total over its own object,
measured at 110 cells. Reporting a round's shape accurately matters more when the accurate
shape is less flattering to the reviewer — which, here, is me.

**And the honest third, for the fourth report running: this is now the story rather than a
caveat.** Three sessions of adversarial rounds on one contract module, all six findings
real, and the module still cannot train anything. I have written the sequencing so the next
block is irreversibly forward — contract closes, trainer gets ONE review, fits run — and if
S81–S88 do not produce a number about whether the model learns, the concern in the S80
progress report stops being a caveat and becomes the result.
